...
namespace KfrDSP1
{
    ...
    template<typename T>
    using EigenColVector  = Eigen::Matrix<T,1,Eigen::Dynamic,Eigen::ColMajor>;
    ...
    struct Complex
    {
        ...
        operator std::complex<T>() { return c; }
    };
    ...
    template<typename T> Complex<T> abs(Complex<T> & c) { return Complex<T>(std::abs(c)); }
    template<typename T> Complex<T> arg(Complex<T> & c) { return Complex<T>(std::arg(c)); }
    template<typename T> Complex<T> norm(Complex<T> & c) { return Complex<T>(std::norm(c)); }
    template<typename T> Complex<T> conj(Complex<T> & c) { return Complex<T>(std::conj(c)); }
    template<typename T> Complex<T> proj(Complex<T> & c) { return Complex<T>(std::proj(c)); }
    template<typename T> Complex<T> polar(Complex<T> & c,const T& t, const T& theta=T()) { return Complex<T>(std::polar(t,theta)); }
    template<typename T> Complex<T> exp(Complex<T> & c) { return Complex<T>(std::exp(c)); }
    template<typename T> Complex<T> log(Complex<T> & c) { return Complex<T>(std::log(c)); }
    template<typename T> Complex<T> log10(Complex<T> & c) { return Complex<T>(std::abs(c)); }
    template<typename T> Complex<T> pow(Complex<T> & c, const Complex<T> & e) { return Complex<T>(std::pow(c,e.c)); }
    template<typename T> Complex<T> pow(Complex<T> & c,const T & e) { return Complex<T>(std::pow(c,e)); }
    template<typename T> Complex<T> sqrt(Complex<T> & c) { return Complex<T>(std::sqrt(c)); }
    template<typename T> Complex<T> sin(Complex<T> & c) { return Complex<T>(std::sin(c)); }
    template<typename T> Complex<T> cos(Complex<T> & c) { return Complex<T>(std::cos(c)); }
    template<typename T> Complex<T> tan(Complex<T> & c) { return Complex<T>(std::tan(c)); }
    template<typename T> Complex<T> asin(Complex<T> & c) { return Complex<T>(std::asin(c)); }
    template<typename T> Complex<T> acos(Complex<T> & c) { return Complex<T>(std::acos(c)); }
    template<typename T> Complex<T> atan(Complex<T> & c) { return Complex<T>(std::atan(c)); }
    template<typename T> Complex<T> sinh(Complex<T> & c) { return Complex<T>(std::sinh(c)); }
    template<typename T> Complex<T> cosh(Complex<T> & c) { return Complex<T>(std::cosh(c)); }
    template<typename T> Complex<T> tanh(Complex<T> & c) { return Complex<T>(std::tanh(c)); }
    template<typename T> Complex<T> asinh(Complex<T> & c) { return Complex<T>(std::asinh(c)); }
    template<typename T> Complex<T> acosh(Complex<T> & c) { return Complex<T>(std::acosh(c)); }
    template<typename T> Complex<T> atanh(Complex<T> & c) { return Complex<T>(std::atanh(c)); }    
    ...
    template<typename T> ArrayMatrix<T> pow(ArrayMatrix<T> & a, const ArrayMatrix<T> & s) { return a.pow(s); }
    ...
    struct Matrix
    {
        ...
        void __setitem(int r, const Vector<T> & v) { 
            if(r < 0) r += rows();
    ...
    struct ColMatrix
    {
        ...
        void __setitem(int r, const ColVector<T> & v) { 
            if(r < 0) r += rows();
            matrix.row(r) = v.vector; 
        }
        void normalize()
    ...
    struct SampleVector
    {
        ...
        SampleVector(const T * ptr, size_t n, size_t chans = 1) {
            vector.resize(n);
            memcpy(vector.data(),ptr,n*sizeof(T));
            channels = chans;
        }
        ...
        void set_channel(const SampleVector<T> & v, size_t ch) {            
            size_t x = 0;
            for(size_t i = ch-1; i < size(); i+=channels) vector[i] = v[x++];
        }
    ...
    {   
        ...
        Window(size_t i) { window.vector.vec.resize(i); }
        virtual ~Window() = default;

        T& operator[](size_t i) { return window[i]; }
        T __getitem(size_t i) { return window[i]; }
        void __setitem(size_t i, T val) { window[i] = val; }
    ...
    struct Hamming: public Window<T>
    {
        Hamming(size_t i) : Window<T>(i) {
            ...
            {
                this->window.vector.vec(i) = 0.54 - (0.46 * std::cos(2*M_PI*i/n));
            }        
        }
    };
    ...
    struct Hanning: public Window<T>
    {
        Hanning(size_t i) : Window<T>(i) {
            ...
            for(size_t i = 0; i < this->window.size(); i++)
            {
                this->window.vector.vec(i) = 0.5*(1 - std::cos(2*M_PI*i/n));
            }        
        }
    };
    ...
    struct Blackman: public Window<T>
    {
        Blackman(size_t i) : Window<T>(i)    
        {
            T n = this->window.vector.size()-1;
            for(size_t i = 0; i < this->window.size(); i++)                    
                this->window.vector.vec(i) = 0.42 - (0.5* std::cos(2*M_PI*i/(n)) + (0.08*std::cos(4*M_PI*i/n)));        
        }
    };
    ...
    struct BlackmanHarris: public Window<T>
    {
        BlackmanHarris(size_t i) : Window<T>(i)    
        {
            T n = this->window.vector.size()-1;
            for(size_t i = 0; i < this->window.size(); i++)            
                this->window.vector.vec(i) = 0.35875 
    ...
    struct Gaussian: public Window<T>
    {
        Gaussian(size_t i) : Window<T>(i)
        {
            ...
            for(size_t n = 0; n < this->window.size(); n++)
            {
                ...
                b = -c * std::sqrt(a);
                this->window.vector.vec(n) = std::exp(b);
            }
        }
    };
    template<typename T>
    struct Welch: public Window<T>
    {
        Welch(size_t i) : Window<T>(i)
        {
            for(size_t i = 0; i < this->window.vector.size(); i++)
                this->window.vector.vec(i) = 1.0 - std::sqrt((2*i-this->window.vector.size())/(this->window.vector.size()+1));        
        }
    };
    template<typename T>
    struct Parzen: public Window<T>
    {
        ...
        {
            for(size_t i = 0; i < this->window.vector.size(); i++)
                this->window.vector.vec(i) = 1.0 - std::abs((2.0f*i-this->window.vector.size())/(this->window.vector.size()+1.0f));        
        }    
    };
    template<typename T>
        ...
        Tukey(size_t i, T alpha) : Window<T>(i)
        {
            ...
            {    
                if(value >= 0 && value <= (alpha * (num_samples/2))) 
                    this->window.vector.vec[i] = 1.0; 
                else if(value <= 0 && (value >= (-1*alpha*(num_samples/2)))) 
                    this->vector.vec[i] = 1.0;
                else 
                    this->vector.vec[i] = 0.5 * (1 + std::cos(M_PI *(((2*value)/(alpha*num_samples))-1)))        ;
                value = value + 1;
            }     
        }
    };
    ...
    {
        size_t half = vector.vec.size() / 2; 
        size_t start= half; 
        if(2*half < vector.vec.size()) start++;
    ...
    T quadratic_peak_pos(SampleVector<T> & vector,size_t pos)
    {
        ...
        if(x0 == pos) return vector.vec(pos) <= vector.vec(x2)? pos:x2;
        if(x2 == pos) return vector.vec(pos) <= vector.vec(x0)? pos:x0;
        s0 = vector.vec(x0);
        s1 = vector.vec(pos);
        s2 = vector.vec(x2);
    ...
    T quadratic_peak_mag(SampleVector<T> & vector,size_t pos)
    {
        ...
        if((T)index == pos) return vector.vec(index);
        x0 = vector.vec(index-1);
        x1 = vector.vec(index);
        x2 = vector.vec(index+1);
    ...
    {        
        ...
        {
            if(high <= low)
                return vector.vec(median);

            if(high == low+1)
            {
                if(vector.vec(low) > vector.vec(high))
                    swap(vector,low,high);
                return vector.vec(median);
            }
            middle = (low+high)/2;
            if(vector.vec(middle) > vector.vec(high)) swap(vector,middle,high);
            if(vector.vec(low) > vector.vec(high)) swap(vector,low,high);
            if(vector.vec(middle) > vector.vec(low)) swap(vector,middle,low);
            ...
            for(;;)
            {
                do ll++; while( vector.vec(low) > vector.vec(ll));
                do hh--; while( vector.vec(hh) > vector.vec(low));
                if(hh < ll) break;
                swap(vector,ll,hh);
            }
            swap(vector,low,hh);
    ...
    T moving_threshold(SampleVector<T> & input, size_t post, size_t pre, size_t pos)
    {
        ...
        if(pos < post+1)
        {
            ...
            for(size_t k = post+1-pos; k < win_length; k++)
                vector.vec(k) = vector.vec(k+pos-post);            
        }
    ...
    {
        T zcr = 0;
        for(size_t i = 1; i < vector.vec.size(); i++)
        {
            bool current = vector.vec(i) > 0;
            bool prev    = vector.vec(i-1) > 0;
            if(current != prev) zcr++;
        }    
        return zcr;   
    }
    ...
    void autocorr(SampleVector<T> & vector,SampleVector<T> & output)
    {
        ...
        for(size_t i = 0; i < vector.size(); i++)
        {
            ...
            for(size_t j = 0; j < vector.size(); j++)
                tmp += vector.vec(j-i) * vector.vec(j);                
            
            output.vector.vec(i) = tmp / (T)(vector.size()-1);
        }
    }
    ...
    {
        for(size_t i = 0; i < vector.vec.size()-1; i++) vector.vec(i) = vector.vec(i+1);
        vector.vec(vector.vec.size()-1) = new_elem;
    }
    ...
    void clamp(SampleVector<T> & vector,T absmax) { 
        for(size_t i = 0; i < vector.size(); i++)
        {
            if( vector.vec(i) > absmax) vector.vec(i) = absmax;
            else if(vector.vec(i) < -absmax) vector.vec(i) = -absmax;
        }
    }

    template<typename T> 
    void  normalize(SampleVector<T> & vector) { 
            Eigen::Matrix<T,1,Eigen::Dynamic> r = vector.vec.matrix();
            r.normalize();
            vector.vec = r.array();         
    }
    ...
    {
        bool tmp = false;
        tmp = (vector.vec(pos) > vector.vec(pos-1)) && (vector.vec(pos) > vector.vec(pos+1)) && vector.vec(pos) > 0;
        return tmp;
    }
    ...
    {
        T sum = vector.vec.pow(2).sum();        
        return std::sqrt(sum/vector.vec.size());
    }
    ...
    T peak_energy(SampleVector<T> & vector)
    {
        T peak = -10000;
        T as;         
        for(size_t i = 0; i < vector.vec.size(); i++)
        {
            T as = std::fabs(vector.vec(i));
            if(as > peak) peak = as;
        }
        return peak;
    }
    ...
    T min(SampleVector<T> & vector) { 
        T min = 1e120;
        for(size_t i = 0; i < vector.size(); i++)
            if(vector.vec(i) < min) min = vector.vec(i);
        return min;            
        //return vector.vec(vector.vec.minCoeff()); 
    }
    ...
    T max(SampleVector<T> & vector) { 
        T max = -1e120;
        for(size_t i = 0; i < vector.size(); i++)
            if(vector.vec(i) > max) max = vector.vec(i);
        return max;
    ...
    {
        out.vector.vec = vector.vec * weight.vector.vec;
        return out;
    }    
    ...
    {
        T tmp = vector.vec.abs().pow(alpha).sum();
    ...
    void alpha_normalize(SampleVector<T> & vector,SampleVector<T> & out, T alpha)
    {
        ...
        out.vector.vec = out.vector.vec / tmp;         
    }    
