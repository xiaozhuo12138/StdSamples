...
typedef void (*activation)(Matrix<ValueType> &  in, Matrix<ValueType> & out);
typedef void (*activation_grad)(Matrix<ValueType> & in, Matrix<ValueType> & out);
...
inline void linear_act(Matrix<ValueType>& input,Matrix<ValueType>& output) {
    output.copy(input);
}
inline void linear_grad_act(Matrix<ValueType>& input, Matrix<ValueType>& output) {
    if(output.size() != input.size()) output.resize(input.rows(),input.cols());
    output.fill(1.0f);
}
inline void softmax_act(Matrix<ValueType>& input, Matrix<ValueType>& output) {
    if(output.size() != input.size()) output.resize(input.rows(),input.cols());
    Viper2::softmax(input,output);    
}
inline void tanh_act(Matrix<ValueType>& input, Matrix<ValueType>& output) {
    if(output.size() != input.size()) output.resize(input.rows(),input.cols());
    Viper2::tanh(input,output);
}
inline void tanh_grad_act(Matrix<ValueType>& input, Matrix<ValueType> & output) {
    if(output.size() != input.size()) output.resize(input.rows(),input.cols());
    Viper2::tanh_deriv(input,output);    
}
inline void sigmoid_act(Matrix<ValueType>& input, Matrix<ValueType>& output) {
    if(output.size() != input.size()) output.resize(input.rows(),input.cols());
    Viper2::sigmoid(input,output);    
}
inline void sigmoid_grad_act(Matrix<ValueType>& input, Matrix<ValueType>& output) {
    if(output.size() != input.size()) output.resize(input.rows(),input.cols());
    Viper2::sigmoid_deriv(input,output);
}
inline void relu_act(Matrix<ValueType>& input, Matrix<ValueType>& output) {
    if(output.size() != input.size()) output.resize(input.rows(),input.cols());
    Viper2::relu(input,output);    
}
inline void relu_grad_act(Matrix<ValueType>& input, Matrix<ValueType>& output) {
    if(output.size() != input.size()) output.resize(input.rows(),input.cols());
    Viper2::relu_deriv(input,output);    
}
...
struct Layer {
    ...
    void Activate(Matrix<ValueType>& tmp) {             
        if(input.size() != tmp.size()) input.resize(tmp.rows(),tmp.cols());        
        activate_f(tmp,input);                        
    }
    void Grad(Matrix<ValueType> & in,Matrix<ValueType> & out) {
        if(out.size() != in.size()) out.resize(in.rows(),in.cols());
        activate_grad_f(in,out);        
    }
...
{                      
    ...
    Viper2::mul_matrix(errorLastTi,errori,weights.t());    
    //fprimei.copy(inputfrom);
    grad(inputfrom,fprimei);    
    //dbi = hadamard(errorLastTi,fprimei);    
    Viper2::hadamard_fast(dbi,errorLastTi,fprimei);
    //dWi = inputto * errori;                                                        
    Viper2::mul_matrix(dWi,inputto,dbi);
    //dbi.copy(errori);        
}
                     ...
                     Matrix<ValueType>& regi, ValueType regularization_strength)
{
    ...
    Viper2::mul_const(dWi_last,momentum_factor,dWi_last);
    Viper2::mul_const(dbi_last,momentum_factor,dbi_last);
    
    //dWi_avg = dWi_avg * clr;            
    //dbi_avg = dbi_avg * clr;              
    Viper2::mul_const(dWi_avg,clr,dWi_avg);
    Viper2::mul_const(dbi_avg,clr,dbi_avg);
        
    //regi    = weights * regularization_strength;                                                                                                                                                
    //dWi_avg = regi + dWi_avg;                       
    Viper2::mul_const(weights,regularization_strength,regi);
    Viper2::add_matrix(dWi_avg,regi,dWi_avg);                        

    //dWi_avg = (dWi_last + dWi_avg);
    //dbi_avg = (dbi_last + dbi_avg);
    Viper2::add_matrix(dWi_last,dWi_last,dWi_avg);
    Viper2::add_matrix(dbi_last,dbi_last,dbi_avg);

    //weights = (-dWi_avg) + weights;            
    //bias    = (-dbi_avg) + bias;                                                    
    //add_matrix(weights,-dWi_avg,weights);
    //add_matrix(bias,-dbi_avg,bias);
    Viper2::sub_matrix(weights,weights,dWi_last);
    Viper2::sub_matrix(bias,bias,dbi_last);
    //dWi_avg.zeros();
    //dbi_avg.zeros();
...
{   
    if(ps.momentum_factor != 0) {            
        Viper2::mul_const(dWi_last,ps.momentum_factor,dWi_last);
        Viper2::mul_const(dbi_last,ps.momentum_factor,dbi_last);
    }
        
    Viper2::mul_const(dWi_avg,alpha,dWi_avg);
    Viper2::mul_const(dbi_avg,alpha,dbi_avg);

    if(ps.regularization_strength != 0) {
        Viper2::mul_const(weights,ps.regularization_strength,regi);
        Viper2::add_matrix(dWi_avg,regi,dWi_avg);                        
    }
 
    if(ps.momentum_factor != 0 or ps.regularization_strength != 0) {
        Viper2::add_matrix(dWi_avg,dWi_last,dWi_avg);
        Viper2::add_matrix(dbi_avg,dbi_last,dbi_avg);
    }
    ...
    vdw = gamma2 * vdw + (1-gamma2) * Viper2::hadamard(dWi_avg,dWi_avg,dWi_avg);  //*/pow(dWi_avg,(ValueType)2.0);    
    vdb = gamma2 * vdb + (1-gamma2) * Viper2::hadamard(dbi_avg,dbi_avg,dbi_avg);  //*/pow(dbi_avg,(ValueType)2.0);
    
    Matrix<ValueType> mdw_corr = sdw / ((ValueType)1 - (ValueType)std::pow(gamma1,epoch+1));
    Matrix<ValueType> mdb_corr = sdb / ((ValueType)1 - (ValueType)std::pow(gamma1,epoch+1));
    Matrix<ValueType> vdw_corr = vdw / ((ValueType)1 - (ValueType)std::pow(gamma2,epoch+1));
    Matrix<ValueType> vdb_corr = vdb / ((ValueType)1 - (ValueType)std::pow(gamma2,epoch+1));

    weights = weights - Viper2::hadamard(alpha / (Viper2::sqrt(vdw_corr + (ValueType)1e-08,vdw_corr)),mdw_corr,mdw_corr);
    bias    = bias    - Viper2::hadamard(alpha / (Viper2::sqrt(vdb_corr + (ValueType)1e-08,vdb_corr)),mdb_corr,mdb_corr);    
...
{                    
    ...
    vdw = gamma * vdw + (1-gamma) * Viper2::hadamard(dWi_avg,dWi_avg,dWi_avg);
    vdb = gamma * vdb + (1-gamma) * Viper2::hadamard(dbi_avg,dbi_avg,dbi_avg);

    weights = weights - Viper2::hadamard((alpha/Viper2::sqrt(vdw + (ValueType)1e-08,vdw)),dWi_avg,dWi_avg);
    bias    = bias    - Viper2::hadamard((alpha/Viper2::sqrt(vdb + (ValueType)1e-08,vdb)),dbi_avg,dbi_avg);
...
struct Network {
    ...
    ValueType CrossEntropyLoss(Matrix<ValueType>& prediction, Matrix<ValueType>& actual, ValueType rs) {
        ...
        ValueType reg_err = 0;        
        total_err = Viper2::sum(actual * log2(prediction,prediction));
        
        if(rs > 0)
            for(size_t i = 0; i < connections.size(); i++)
            {
                Matrix<ValueType> &weights = connections[i]->weights;
                reg_err += Viper2::sum(Viper2::sqr(weights));
            }
    ...
    ValueType MeanSquaredError(Matrix<ValueType>& prediction, Matrix<ValueType> & actual, ValueType rs) {
        ...
        if(rs > 0)
            for(size_t i = 0; i < connections.size(); i++)
            {
                ...
                reg_err += Viper2::sum(Viper2::sqr(w));
            }
    ...
    void gd_update(ParameterSet& ps, size_t epoch) {
        Matrix<ValueType> & data = ps.data;
        Matrix<ValueType> & classes = ps.classes;
        ValueType currentLearningRate = ps.learning_rate;
        if(ps.search_time != 0) {
    ...
    void update(ParameterSet& ps, size_t epoch) {
        switch(ps.optimizer) {
            case ParameterSet::GD : gd_update(ps,epoch); break;
            case ParameterSet::ADAM: adam_update(ps,epoch); break;
            case ParameterSet::RMSPROP: rmsprop_update(ps,epoch); break;
        }
    }
    ...
    void optimize(ParameterSet & ps)
    {
        switch(ps.batchmode)
        {
            case ParameterSet::BATCH: batch(ps); break;
            case ParameterSet::NOBATCH: train(ps); break;
            case ParameterSet::REVERSEBATCH: reverse_batch(ps); break;
            case ParameterSet::BATCHALL: batch_all(ps); break;
        }
    }
};
...
void XOR(ActivationType atype, ValueType lt, ValueType mf)
{
    ...
    Matrix<ValueType> e = matrix_new(4,2,examples_bp);
    Matrix<ValueType> t = matrix_new(4,1,training_bp);
