...
inline DspFloatType dLinTerp(DspFloatType x1, DspFloatType x2, DspFloatType y1, DspFloatType y2, DspFloatType x)
{
...
inline bool normalizeBuffer(DspFloatType* pInputBuffer, uint32_t uBufferSize)
{
	DspFloatType fMax = 0;

	for(uint32_t j=0; j<uBufferSize; j++)
	{
...
struct modMatrixRow
{
	...
	uint32_t uDestinationIndex;

	// --- for GUI -> Control
	DspFloatType dUIContolValue;

	// --- needs to default to 1.0 in case no GUI control
	DspFloatType* pModIntensity;	// <- "ucControl" in MMA DLS
	DspFloatType* pModRange;		// <- "lScale" in MMA DLS

	// --- transform on Source
	uint32_t uSourceTransform;
...
struct noteRegion
{
	...
	uint32_t uMIDINoteLow;
	uint32_t uMIDINoteHigh;

	// velocity range of the Region
	uint32_t uMIDIVelocityLow;
	uint32_t uMIDIVelocityHigh;
};
...
inline modMatrixRow* createModMatrixRow(uint32_t uSource, uint32_t uDestination, DspFloatType* pIntensity, DspFloatType* pRange, uint32_t uTransform, bool bEnable = true)
{
...
struct globalOscillatorParams
{
	// --- common
	DspFloatType dOscFo;
	DspFloatType dFoRatio;
	DspFloatType dAmplitude;	// 0->1 from GUI
	DspFloatType dPulseWidthControl; // from GUI
	int nOctave;			// octave tweak
	int nSemitones;		// semitones tweak
	int nCents;			// cents tweak
	uint32_t uWaveform;		// to store type

	// --- LFOs
	uint32_t uLFOMode;		// to store MODE
...
struct globalFilterParams
{
	...
	DspFloatType dFcControl;
	DspFloatType dQControl;
	DspFloatType dAuxControl;
	DspFloatType dSaturation;
	uint32_t uFilterType;		
	uint32_t uNLP;
};

struct globalEGParams
{
	DspFloatType dAttackTime_mSec;	// att: is a time duration
	DspFloatType dDecayTime_mSec;	// dcy: is a time to decay 1->0
	DspFloatType dReleaseTime_mSec;	// rel: is a time to decay 1->0
	DspFloatType dSustainLevel;
	DspFloatType dShutdownTime_mSec; // shutdown is a time
	bool bResetToZero;
	bool bLegatoMode;
};
...
{
	DspFloatType dAmplitude_dB;		// the user's control setting in dB
	DspFloatType dPanControl;
};
...
struct globalVoiceParams
{
	// --- common
	uint32_t uVoiceMode;
	DspFloatType dHSRatio;
	DspFloatType dPortamentoTime_mSec;

	// --- ranges
	DspFloatType dOscFoPitchBendModRange;
	DspFloatType dFilterModRange;
	DspFloatType dAmpModRange;
	DspFloatType dOscFoModRange;
	DspFloatType dOscHardSyncModRange;

	// --- intensities
	DspFloatType dFilterKeyTrackIntensity;

	DspFloatType dLFO1OscModIntensity;
	DspFloatType dLFO1HSModIntensity;
	DspFloatType dLFO1Filter1ModIntensity;
	DspFloatType dLFO1Filter2ModIntensity;	
	DspFloatType dLFO1DCAAmpModIntensity;
	DspFloatType dLFO1DCAPanModIntensity;

	DspFloatType dLFO2OscModIntensity;
	DspFloatType dLFO2HSModIntensity;
	DspFloatType dLFO2Filter1ModIntensity;
	DspFloatType dLFO2Filter2ModIntensity;	
	DspFloatType dLFO2DCAAmpModIntensity;
	DspFloatType dLFO2DCAPanModIntensity;

	DspFloatType dEG1OscModIntensity;
	DspFloatType dEG1Filter1ModIntensity;
	DspFloatType dEG1Filter2ModIntensity;
	DspFloatType dEG1DCAAmpModIntensity;

	DspFloatType dEG2OscModIntensity;
	DspFloatType dEG2Filter1ModIntensity;
	DspFloatType dEG2Filter2ModIntensity;
	DspFloatType dEG2DCAAmpModIntensity;

	DspFloatType dEG3OscModIntensity;
	DspFloatType dEG3Filter1ModIntensity;
	DspFloatType dEG3Filter2ModIntensity;
	DspFloatType dEG3DCAAmpModIntensity;

	DspFloatType dEG4OscModIntensity;
	DspFloatType dEG4Filter1ModIntensity;
	DspFloatType dEG4Filter2ModIntensity;
	DspFloatType dEG4DCAAmpModIntensity;

	// --- vector synth
	DspFloatType dOrbitXAmp;
	DspFloatType dOrbitYAmp;
	DspFloatType dAmplitude_A;
	DspFloatType dAmplitude_B;
	DspFloatType dAmplitude_C;
	DspFloatType dAmplitude_D;
	DspFloatType dAmplitude_ACmix;
	DspFloatType dAmplitude_BDmix;
	uint32_t uVectorPathMode;

	// --- DX synth
	DspFloatType dOp1Feedback;
	DspFloatType dOp2Feedback;
	DspFloatType dOp3Feedback;
	DspFloatType dOp4Feedback;
};
...
inline DspFloatType calculateDXAmplitude(DspFloatType dDXLevel)
{
	DspFloatType dDXAmplitude = 0.0;
	...
	if(dDXLevel != 0.0)
	{
		...
		// set dB-ish version
		dDXAmplitude = (std::pow(10.0, dDXAmplitude/20.0));
	}
...
inline DspFloatType cookVSTGUIVariable(DspFloatType fMin, DspFloatType fMax, DspFloatType fVar)
{
	return (fMax - fMin)*fVar + fMin;
}

// returns normalized version of fVar
inline DspFloatType convertToVSTGUIVariable(DspFloatType fMin, DspFloatType fMax, DspFloatType fVar)
{
	DspFloatType fDiff = fMax - fMin;
	DspFloatType fRawData = (fVar - fMin)/fDiff;
	return fRawData;
}
...
static inline DspFloatType fastlog2 (DspFloatType x)
{
  union { DspFloatType f; unsigned int i; } vx = { x };
  union { unsigned int i; DspFloatType f; } mx = { (vx.i & 0x007FFFFF) | 0x3f000000 };
  DspFloatType y = vx.i;
...
static inline DspFloatType fastpow2 (DspFloatType p)
{
  DspFloatType offset = (p < 0) ? 1.0f : 0.0f;
  DspFloatType clipp = (p < -126) ? -126.0f : p;
  int w = clipp;
  DspFloatType z = clipp - w + offset;
  union { unsigned int i; DspFloatType f; } v = { static_cast<unsigned int> ( (1 << 23) * (clipp + 121.2740575f + 27.7280233f / (4.84252568f - z) - 1.49012907f * z) ) };

  return v.f;
}

static inline DspFloatType fastexp (DspFloatType p)
{
  return fastpow2 (1.442695040f * p);
}


static inline DspFloatType fasttan (DspFloatType x)
{
  static const DspFloatType halfpi = 1.5707963267948966f;
  return std::sin (x) / std::sin (x + halfpi);
}

static inline DspFloatType fasttanh (DspFloatType p)
{
//  return -1.0f + 2.0f / (1.0f + fastexp (-2.0f * p));
  return p/(fabs(2*p)+3/(2+2*p*2*p));
}

inline DspFloatType fastPow(DspFloatType a, DspFloatType b) {
  union {
	DspFloatType d;
	int x[2];
  } u = { a };
...
inline DspFloatType semitonesBetweenFrequencies(DspFloatType dStartFrequency, DspFloatType dEndFrequency)
{
...
*/
inline DspFloatType pitchShiftMultiplier(DspFloatType dPitchShiftSemitones)
{
...
*/
inline DspFloatType paramToTimeCents(DspFloatType dParam)
{
...
/* timeCentsToParam()
	...
	See the MMA MIDI DLS Level 1 or 2 Spec
*/
inline DspFloatType timeCentsToParam(DspFloatType dTimeCents)
{
	//return fastpow2(dTimeCents/(1200.0*65536.0));
	// return fastpow2(dTimeCents/(1200.0*65536.0));
	return std::pow(2.0, dTimeCents/(1200.0*65536.0));
}
...
/* mmaMIDItoParamScaling()
	...
	Examples:
		...
		Key to EG Decay Time: decay shortens as MIDI note number increases
							  ...
							  fScaling = Vel to EG Attack Scale
*/
inline DspFloatType mmaMIDItoParamScaling(uint32_t uMIDIValue, DspFloatType fParameter, DspFloatType fScaling)
{
	return timeCentsToParam(paramToTimeCents(fScaling)*(DspFloatType(uMIDIValue)/128.0) + paramToTimeCents(fParameter));
}
...
*/
inline DspFloatType mmaMIDItoAtten_dB(uint32_t uMIDIValue)
{
	if(uMIDIValue == 0)
		return -96.0; // dB floor

	return 20.0*log10((127.0*127.0)/((DspFloatType)uMIDIValue*(DspFloatType)uMIDIValue));
}
...
/* mmaMIDItoAtten()
	...
	uMIDIValue = the MIDI (0 -> 127) value to convert
*/
inline DspFloatType mmaMIDItoAtten(uint32_t uMIDIValue)
{
	if(uMIDIValue == 0)
		return 0.0; // floor

	return ((DspFloatType)uMIDIValue*(DspFloatType)uMIDIValue)/(127.0*127.0);;
}
...
*/
inline DspFloatType capChargeTransform(DspFloatType dValue)
{
	if(dValue == 0.0)
		return 0.0;

	return 1.0 - std::exp(-6.0*dValue);
}
...
/* capDischargeTransform()
	...
	dValue = the unipolar (0 -> 1) value to convert
*/
inline DspFloatType capDischargeTransform(DspFloatType dValue)
{
	if(dValue == 1.0)
		return 0.0;

	return std::exp(-6.0*dValue);
}
...
/* linearIn_dBTransform()
	...
	dValue = the unipolar (0 -> 1) value to convert
*/
inline DspFloatType linearIn_dB_AttackTransform(DspFloatType dValue)
{
	if(dValue == 0.0)
		return 0.0;

	return std::pow(10.0, ((1.0 - dValue)*-96.0)/20.0);
}
...
/* linearIn_dBTransform()
	...
	dValue = the unipolar (0 -> 1) value to convert
*/
inline DspFloatType linearIn_dB_DecayTransform(DspFloatType dValue)
{
	if(dValue == 1.0)
		return 0.0;

	return std::pow(10.0, (dValue*-96.0)/20.0);
}
...
/* convexTransform()
	...
	dValue = the unipolar (0 -> 1) value to convert
*/
inline DspFloatType convexTransform(DspFloatType dValue)
{
...
/* convexInvertedTransform()
	...
	dValue = the unipolar (0 -> 1) value to convert
*/
inline DspFloatType convexInvertedTransform(DspFloatType dValue)
{
...
/* concaveTransform()
	...
	dValue = the unipolar (0 -> 1) value to convert
*/
inline DspFloatType concaveTransform(DspFloatType dValue)
{
	if(dValue >= CONCAVE_LIMIT)
		return 1.0;
...
/* concaveInvertedTransform()
	...
	dValue = the unipolar (0 -> 1) value to convert
*/
inline DspFloatType concaveInvertedTransform(DspFloatType dValue)
{
	if(dValue <= CONVEX_LIMIT)
		return 1.0;
...
*/
inline DspFloatType unipolarToBipolar(DspFloatType dValue)
{
...
/* midiToBipolar()
	...
	uint32_t uMIDIValue = the MIDI value to convert
*/
inline DspFloatType midiToBipolar(uint32_t uMIDIValue)
{
	return 2.0*(DspFloatType)uMIDIValue/127.0 - 1.0;
}
...
/* midiToPanValue()
	...
	uint32_t uMIDIValue = the MIDI value to convert
*/
inline DspFloatType midiToPanValue(uint32_t uMIDIValue)
{
...
/* bipolarToUnipolar()
	...
	dValue = the value to convert
*/
inline DspFloatType bipolarToUnipolar(DspFloatType dValue)
{
...
/* midiToUnipolar()
	...
	dValue = the value to convert
*/
inline DspFloatType midiToUnipolar(uint32_t uMIDIValue)
{
	return (DspFloatType)uMIDIValue/127.0;
}
...
/* unipolarToMIDI()
	...
	dValue = the value to convert
*/
inline uint32_t unipolarToMIDI(DspFloatType fUnipolarValue)
{
...
*/
inline void calculatePanValues(DspFloatType dPanMod, DspFloatType& dLeftPanValue, DspFloatType& dRightPanValue)
{
	//dLeftPanValue = fastcos((M_PI/4.0)*(dPanMod + 1.0));
	//dRightPanValue = fastsin((M_PI/4.0)*(dPanMod + 1.0));

	dLeftPanValue = std::cos((M_PI/4.0)*(dPanMod + 1.0));
	dRightPanValue = std::sin((M_PI/4.0)*(dPanMod + 1.0));

	dLeftPanValue = fmax(dLeftPanValue, (DspFloatType)0.0);
	dLeftPanValue = fmin(dLeftPanValue, (DspFloatType)1.0);
	dRightPanValue = fmax(dRightPanValue, (DspFloatType)0.0);
	dRightPanValue = fmin(dRightPanValue, (DspFloatType)1.0);
}
...
*/
inline DspFloatType calcValueVoltOctaveExp(DspFloatType dLowLimit, DspFloatType dHighLimit, DspFloatType dControlValue)
{
	DspFloatType dOctaves = fastlog2(dHighLimit/dLowLimit);
	if(dLowLimit == 0)
...
*/
inline DspFloatType calcInverseValueVoltOctaveExp(DspFloatType dLowLimit, DspFloatType dHighLimit, DspFloatType dControlValue)
{
	DspFloatType dOctaves = fastlog2(dHighLimit/dLowLimit);
	if(dLowLimit == 0)
		return dControlValue;
...
inline DspFloatType log2( DspFloatType n )  
{  
	// std::log(n)/std::log(2) is log2.  
	return std::log( n ) / std::log((DspFloatType)2 );  
}
...
*/
inline DspFloatType calcModulatedValueVoltOctaveExp(DspFloatType dLowLimit, DspFloatType dHighLimit, DspFloatType dControlValue, 
											  DspFloatType dModValue, bool bExpUserControl)
{
	dModValue = bipolarToUnipolar(dModValue);

//	DspFloatType dOctaves = fastlog2(dHighLimit/dLowLimit);
	DspFloatType dOctaves = log2(dHighLimit/dLowLimit);
	
	// location of control relative to 50% point
	DspFloatType dControlOffset = bExpUserControl ? calcInverseValueVoltOctaveExp(dLowLimit, dHighLimit,dControlValue) - 0.5 : ((dControlValue - dLowLimit)/(dHighLimit - dLowLimit)) - 0.5;

	// std::exp control
//	DspFloatType dExpControlOffset = dLowLimit*fastpow2(dControlOffset*dOctaves);
	DspFloatType dExpControlOffset = dLowLimit*std::pow(2.0, dControlOffset*dOctaves);
	
	// mod std::exp control
	DspFloatType dModulatedValueExp = dExpControlOffset*std::pow(2.0, dModValue*dOctaves);
...
/* calcModulatedValueLin()
	...
	dModValue = moudulator: on the range of 0 -> 1
*/
inline DspFloatType calcModulatedValueLin(DspFloatType dLowLimit, DspFloatType dHighLimit, DspFloatType dControlValue, DspFloatType dModValue)
{	
	// convert mod value to bipolar
	DspFloatType dModulator = unipolarToBipolar(dModValue)*(dHighLimit - dLowLimit)/2.0;// + dLowLimit; 
	
	DspFloatType dModulatedValue = dControlValue + dModulator;
...
inline DspFloatType doWhiteNoise()
{
	DspFloatType fNoise = 0.0;
	
#if defined _WINDOWS || defined _WINDLL    
	// fNoise is 0 -> 32767.0
	fNoise = (DspFloatType)rand();

	// normalize and make bipolar
	fNoise = 2.0*(fNoise/32767.0) - 1.0;
#else
	// fNoise is 0 -> ARC4RANDOMMAX
	fNoise = (DspFloatType)rand() / (DspFloatType) RAND_MAX; // ???(DspFloatType)arc4random();
...
inline DspFloatType doPNSequence(uint32_t& uPNRegister)
{
	// get the bits
	...
	DspFloatType fOut = (DspFloatType)(uPNRegister)/((std::pow((DspFloatType)2.0,(DspFloatType)32.0))/16.0);
...
const DspFloatType B = 4.0/(DspFloatType)M_PI;
const DspFloatType C = -4.0/((DspFloatType)M_PI*(DspFloatType)M_PI);
const DspFloatType P = 0.225;
// http://devmaster.net/posts/9648/fast-and-accurate-sine-cosine
// input is -M_PI to +M_PI
inline DspFloatType parabolicSine(DspFloatType x, bool bHighPrecision = true)
{
	DspFloatType y = B * x + C * x * fabs(x);
...
const DspFloatType D = 5.0*(DspFloatType)M_PI*(DspFloatType)M_PI;
inline DspFloatType BhaskaraISine(DspFloatType x)
{
	DspFloatType d = fabs(x);
	DspFloatType sgn = d == 0 ? 1.0 : x/fabs(x);
	return 16.0*x*((DspFloatType)M_PI - sgn*x)/(D - sgn*4.0*x*((DspFloatType)M_PI - sgn*x));
}
...
inline void copyWaveData(CWaveData* pSourceWaveData, CWaveData* pDestWaveData)
{
	...
	pDestWaveData->m_pWaveBuffer = new DspFloatType[pSourceWaveData->m_uSampleCount];
...
inline void RotatePoint45CW(DspFloatType& x1, DspFloatType& y1)
{
	DspFloatType x = x1;
	DspFloatType y = y1;
	x1 = (x + y)/1.41421356;
...
inline void RotatePoint45CCW(DspFloatType& x1, DspFloatType& y1)
{
	DspFloatType x = x1;
	DspFloatType y = y1;
	x1 = (x - y)/1.41421356;
...
inline void calculateVectorMixValues(DspFloatType dOriginX, DspFloatType dOriginY, 
									 DspFloatType dPointX, DspFloatType dPointY, 
									 DspFloatType& dAmag, DspFloatType& dBmag, DspFloatType& dCmag, DspFloatType& dDmag, 
									 DspFloatType& dACMix, DspFloatType& dBDMix, int nCellsPerSide, bool bJoystickCoords = true)
{
	DspFloatType x = dPointX;
	DspFloatType y = dPointY;

	DspFloatType dUnitScalar = 1.41421356*nCellsPerSide; // ncells*sqrt(2)	
...
inline void calculateVectorJoystickValues(DspFloatType dPointX, DspFloatType dPointY, 
										  DspFloatType& dAmag, DspFloatType& dBmag, DspFloatType& dCmag, DspFloatType& dDmag, 
										  DspFloatType& dACMix, DspFloatType& dBDMix)
{
	// these are 0->1 unipolar
...
// return cell
inline int translateRotorPointToGridCell(DspFloatType& dOriginX, DspFloatType& dOriginY, 
										 DspFloatType& dRotorX, DspFloatType& dRotorY,
										 int nCellsPerSide, 
										 bool bJoystickCoords = true)
{
	...
	// now the testing
	DspFloatType yL = -nCellsPerSide + 2.0*(nCellsPerSide - 1);
	DspFloatType yH = nCellsPerSide;

	DspFloatType xL = -nCellsPerSide;
	DspFloatType xH = nCellsPerSide - 2.0*(nCellsPerSide - 1);

	dRotorX = fmax(dRotorX, (DspFloatType)-nCellsPerSide);
	dRotorX = fmin(dRotorX, (DspFloatType)nCellsPerSide);

	dRotorY = fmax(dRotorY, (DspFloatType)-nCellsPerSide);
	dRotorY = fmin(dRotorY, (DspFloatType)nCellsPerSide);
...
// 2-sample PolyBLEP
inline DspFloatType doPolyBLEP_2(DspFloatType dModulo, DspFloatType dInc, DspFloatType dHeight, bool bRisingEdge)
{
	// --- return value
	DspFloatType dPolyBLEP = 0.0;
	...
	{ 		
		// --- calculate distance
		DspFloatType t = (dModulo - 1.0)/dInc; 
...
inline DspFloatType doBLEP_N(const DspFloatType* pBLEPTable, DspFloatType dTableLen, DspFloatType dModulo, DspFloatType dInc, DspFloatType dHeight, 
					   bool bRisingEdge, DspFloatType dPointsPerSide, bool bInterpolate = false)
{
	// return value
	DspFloatType dBLEP = 0.0;

	// t = the distance from the discontinuity
	DspFloatType t = 0.0;

	// --- find the center of table (discontinuity location)
	DspFloatType dTableCenter = dTableLen/2.0 - 1;
	
	// LEFT side of edge
	// -1 < t < 0
	for(int i = 1; i <= (uint32_t)dPointsPerSide; i++)
	{
		if(dModulo > 1.0 - (DspFloatType)i*dInc)
		{ 		
			// --- calculate distance
			t = (dModulo - 1.0)/(dPointsPerSide*dInc); 
			
			// --- get index
			DspFloatType fIndex = (1.0 + t)*dTableCenter;
			...
			{
				DspFloatType fIndex = (1.0 + t)*dTableCenter;
				DspFloatType frac = fIndex - int(fIndex);
	...
	for(int i = 1; i <= (uint32_t)dPointsPerSide; i++)
	{
		if(dModulo < (DspFloatType)i*dInc)
		{ 
			// calculate distance
			t = dModulo/(dPointsPerSide*dInc); 
			
			// --- get index
			DspFloatType fIndex = t*dTableCenter + (dTableCenter + 1.0);
