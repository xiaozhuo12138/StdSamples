...
inline modMatrixRow* createModMatrixRow(UINT uSource, UINT uDestination, DspFloatType* pIntensity, DspFloatType* pRange, UINT uTransform, bool bEnable = true)
{
...
/* mmaMIDItoParamScaling()
	...
	Examples:
		...
		Key to EG Decay Time: decay shortens as MIDI note number increases
							  ...
							  fScaling = Vel to EG Attack Scale
*/
inline DspFloatType mmaMIDItoParamScaling(UINT uMIDIValue, DspFloatType fParameter, DspFloatType fScaling)
{
...
*/
inline DspFloatType mmaMIDItoAtten_dB(UINT uMIDIValue)
{
	if(uMIDIValue == 0)
...
/* mmaMIDItoAtten()
	...
	uMIDIValue = the MIDI (0 -> 127) value to convert
*/
inline DspFloatType mmaMIDItoAtten(UINT uMIDIValue)
{
	if(uMIDIValue == 0)
		return 0.0; // floor
...
*/
inline DspFloatType midiToBipolar(UINT uMIDIValue)
{
...
/* midiToPanValue()
	...
	UINT uMIDIValue = the MIDI value to convert
*/
inline DspFloatType midiToPanValue(UINT uMIDIValue)
{
...
/* midiToUnipolar()
	...
	dValue = the value to convert
*/
inline DspFloatType midiToUnipolar(UINT uMIDIValue)
{
...
/* unipolarToMIDI()
	...
	dValue = the value to convert
*/
inline UINT unipolarToMIDI(DspFloatType fUnipolarValue)
{
...
{  
	// log(n)/log(2) is log2.  
	return log( n ) / log((DspFloatType)2 );  
}
...
//	DspFloatType dExpControlOffset = dLowLimit*fastpow2(dControlOffset*dOctaves);
	DspFloatType dExpControlOffset = dLowLimit*pow(2.0, dControlOffset*dOctaves);
	
	// mod exp control
	DspFloatType dModulatedValueExp = dExpControlOffset*pow(2.0, dModValue*dOctaves);
...
inline DspFloatType doWhiteNoise()
{
	...
	fNoise = (DspFloatType)arc4random();
...
inline DspFloatType doPNSequence(UINT& uPNRegister)
{
	// get the bits
	...
	DspFloatType fOut = (DspFloatType)(uPNRegister)/((pow((DspFloatType)2.0,(DspFloatType)32.0))/16.0);
...
const DspFloatType B = 4.0/(DspFloatType)pi;
const DspFloatType C = -4.0/((DspFloatType)pi*(DspFloatType)pi);
...
const DspFloatType D = 5.0*(DspFloatType)pi*(DspFloatType)pi;
inline DspFloatType BhaskaraISine(DspFloatType x)
{
	DspFloatType d = fabs(x);
	DspFloatType sgn = d == 0 ? 1.0 : x/fabs(x);
	return 16.0*x*((DspFloatType)pi - sgn*x)/(D - sgn*4.0*x*((DspFloatType)pi - sgn*x));
}
