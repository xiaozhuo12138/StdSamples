...
// -----------------------------------------------------------------------------
	...
	bool VA1Filter::reset(DspFloatType _sampleRate)
	{
	...
	void VA1Filter::setFilterParams(DspFloatType _fc, DspFloatType _Q)
	{
		if (fc != _fc)
		{
			fc = _fc;
	...
	{
		DspFloatType g = tan(kTwoPi*fc*halfSamplePeriod); // (2.0 / T)*tan(wd*T / 2.0);
		coeffs.alpha = g / (1.0 + g);

		return true;
	}

	FilterOutput* VA1Filter::process(DspFloatType xn)
	{
		// --- create vn node
		DspFloatType vn = (xn - sn)*coeffs.alpha;
	...
	/** reset members to initialized state */
	bool VASVFilter::reset(DspFloatType _sampleRate)
	{
		sampleRate = _sampleRate;
		halfSamplePeriod = 1.0 / (2.0 * sampleRate);
		integrator_z[0] = 0.0;
	...
	void VASVFilter::setFilterParams(DspFloatType _fc, DspFloatType _Q)
	{
	...
	bool VASVFilter::update()
	{
		...
		DspFloatType theta_c = (sampleRate / 2.0) / fc;
		coeffs.sigma = 1.0 / (coeffs.alpha*theta_c*theta_c);
	
		return true;
	}

	FilterOutput* VASVFilter::process(DspFloatType xn)
	{
	...
	/** reset members to initialized state */
	bool VAKorg35Filter::reset(DspFloatType _sampleRate)
	{
		sampleRate = _sampleRate;
		halfSamplePeriod = 1.0 / (2.0 * sampleRate);
	...
	void VAKorg35Filter::setFilterParams(DspFloatType _fc, DspFloatType _Q)
	{
		// --- use mapping function for Q -> K
	...
	FilterOutput* VAKorg35Filter::process(DspFloatType xn)
	{
		// --- lowpass
		...
		// --- form S35
		DspFloatType S35 = lpfVAFilters[FLT2].getFBOutput() + lpfVAFilters[FLT3].getFBOutput();

		// --- calculate u
		DspFloatType u = coeffs.alpha0*(tempOut->filter[ANM_LPF1] + S35);
	...
	/** reset members to initialized state */
	bool VAMoogFilter::reset(DspFloatType _sampleRate)
	{
		sampleRate = _sampleRate;
		halfSamplePeriod = 1.0 / (2.0 * sampleRate);
	...
	void VAMoogFilter::setFilterParams(DspFloatType _fc, DspFloatType _Q)
	{
		// --- use mapping function for Q -> K
	...
	bool VAMoogFilter::update()
	{
		...
		DspFloatType kernel = 1.0 / (1.0 + coeffs.g);
	...
	FilterOutput* VAMoogFilter::process(DspFloatType xn)
	{
		// --- 4th order MOOG:
		DspFloatType sigma = 0.0;
		...
		DspFloatType u = coeffs.alpha0*(xn - coeffs.K * sigma);
	...
	/** reset members to initialized state */
	bool VADiodeSubFilter::reset(DspFloatType _sampleRate)
	{
		sampleRate = _sampleRate;
		halfSamplePeriod = 1.0 / (2.0 * sampleRate);
		sn = 0.0;
	...
	void VADiodeSubFilter::setFilterParams(DspFloatType _fc, DspFloatType _Q)
	{
		if (fc != _fc)
		{
			fc = _fc;
	...
	{
		DspFloatType g = tan(kTwoPi*fc*halfSamplePeriod);
		coeffs.alpha = g / (1.0 + g);

		return true;
	}

	FilterOutput* VADiodeSubFilter::process(DspFloatType xn)
	{
		// --- specific to diode
		xn = (xn * coeffs.gamma) + feedbackIn + (coeffs.epsilon * getFBOutput());

		// --- v(n)
		DspFloatType vn = ((coeffs.alpha0 * xn) - sn) * coeffs.alpha;
	...
	/** reset members to initialized state */
	bool VADiodeFilter::reset(DspFloatType _sampleRate)
	{
		sampleRate = _sampleRate;
		halfSamplePeriod = 1.0 / (2.0 * sampleRate);
	...
	void VADiodeFilter::setFilterParams(DspFloatType _fc, DspFloatType _Q)
	{
		// --- use mapping function for Q -> K
	...
	const DspFloatType fgn_a = 1.005381;
	const DspFloatType fgn_b = 0.8783896;
	const DspFloatType fgn_c = 1.113067;
	const DspFloatType fgn_d = -0.2110344;

	bool VADiodeFilter::update()
	{
		DspFloatType g = tan(kTwoPi*fc*halfSamplePeriod);

		DspFloatType G4 = 0.5*g / (1.0 + g);
		DspFloatType G3 = 0.5*g / (1.0 + g - 0.5*g*G4);
		DspFloatType G2 = 0.5*g / (1.0 + g - 0.5*g*G3);
		DspFloatType G1 = g / (1.0 + g - g*G2);
		...
		// --- analog 
		DspFloatType f = kTwoPi*fc / sampleRate;
		DspFloatType e = pow((f / fgn_c), fgn_b);
		coeffs.alpha1 = fgn_d + (fgn_a - fgn_d) / (1.0 + e);

		// --- diode sub-filter coeffs
		// 
		// -- alphas
		DspFloatType alpha = g / (1.0 + g);
	...
	FilterOutput* VADiodeFilter::process(DspFloatType xn) 
	{
		...
		// form input
		DspFloatType sigma = coeffs.beta[0] * subFilter[FLT1].getFBOutput() +
		...
		// form input
		DspFloatType u = (xn - (coeffs.K * sigma)) * coeffs.alpha0;
