...
namespace Filters
{
    ...
    struct HighpassChebyshev2
    {
        ...
        HighpassChebyshev2(size_t O,DspFloatType cutoff, DspFloatType sr)
        {
            fs = sr;            
            order = O;
            setCutoff(cutoff);
        ...
        {            
            ff = f/fs;
            filter.coef = spuce::iir_coeff(order,spuce::filter_type::high);         
            spuce::chebyshev2_iir(filter.coef,ff,x);
