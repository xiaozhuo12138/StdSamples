...
namespace FX::Delays::KHDelays
{
    ...
    // passed to function
    //-------------------------------------------------------------------------
    inline DspFloatType BasicDelayLine::numSamplesFromMSf(const int sr, const DspFloatType d_ms){
        return sr * d_ms * .001;
    }
    ...
    struct KHBipodDelay : public MonoFXProcessor
    {
        ...
        DspFloatType Tick(DspFloatType I, DspFloatType A=1, DspFloatType X=0, DspFloatType Y=0)
        {
            DspFloatType x = delay.next(I + feedback*last);            
            if( I < 0) last=x;            
            return last;               
        }
    };
    struct KHDualFeedDelay : public MonoFXProcessor
    {
        ...
        DspFloatType Tick(DspFloatType I, DspFloatType A=1, DspFloatType X=0, DspFloatType Y=0)
        {
            DspFloatType x = feed.Tick(I);
            DspFloatType r = delay.Tick(mix*I + (1-mix)*x);
            return r;            
        }
    };
    ...
    struct KHStereoDelay : public StereoFXProcessor
    {
        ...
        KHStereoDelay(const int sr, const DspFloatType d_ms, const DspFloatType fb, const DspFloatType mix) :
        StereoFXProcessor(),
    ...
    {
        KHMultiDelay<N> left,right;

        KHMultiStereoDelay(const int sr, const DspFloatType d_ms, const DspFloatType fb, const DspFloatType mix) :
        StereoFXProcessor(),
        left(sr,d_ms,fb,mix),
