...
namespace FX
{
    inline Filters::FilterCoefficients makeParametricEQ( DspFloatType discreteFrequency,
                                                                DspFloatType Q,
                                                                DspFloatType gainFactor) 
    {
        ...
        const DspFloatType bandwidth = std::min((DspFloatType)(discreteFrequency / Q),(DspFloatType)( M_PI * 0.99));
    ...
    {
        DspFloatType centreFrequency_, q_, gainDecibels_;    
        Filters::BiquadFilter **eqFilters_;
        int numEqFilters_;
        ...
        // Set default values:
            ...
            eqFilters_ = (Filters::BiquadFilter**)malloc(numEqFilters_ * sizeof(Filters::BiquadFilter*));
            if(eqFilters_ == 0)
            numEqFilters_ = 0;
            else {
            for(int i = 0; i < numEqFilters_; i++)
                eqFilters_[i] = new Filters::BiquadFilter(sampleRate);
            }
