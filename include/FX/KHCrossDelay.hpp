...
namespace FX::Delays::KHDelays
{
    ...
    inline DspFloatType CrossDelayLine::numSamplesFromMSf(const int sr, const DspFloatType d_ms){
        return sr * d_ms * .001;
    }
    ...
    //----------------------------------------------------------------------
    inline DspFloatType CrossDelayLine::linInterp(DspFloatType x1, DspFloatType x2, DspFloatType y1, DspFloatType y2, DspFloatType x){
    ...
    struct KHCrossDelay : public FunctionProcessor
    {
        CrossDelayLine delay;

        KHCrossDelay(const int sr, const DspFloatType d_ms, const DspFloatType fb, const DspFloatType mix, const bool cross)
        : FunctionProcessor(),
          delay(sr,d_ms,fb,mix,cross)
        {

        }
            
        void setTimeMS(DspFloatType ms) {
            delay.setDelayTimeMS(sampleRate,ms);
        }
        ...
        void setCurrentFeedbackInput(DspFloatType f)
        {
            delay.setCurrentFeedbackInput(f);
        }
        void setUseExternalFeedback(bool b) {
            delay.setUseExternalFeedback(b);
        }
        void setCrossedFeedback(bool b) {
            delay.setCrossedFeedback(b);
        }

        DspFloatType Tick(DspFloatType in, DspFloatType A, DspFloatType X, DspFloatType Y) {
