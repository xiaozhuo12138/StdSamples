...
namespace Filters::AudioTK::Dynamics
{
    struct GainFilter : public ATKFilter
    {
        ...
        void setPort(int port, DspFloatType value) {
            switch(port) {
                case PORT_THRESHOLD: filter->set_threshold(value); break;
                case PORT_THRESHOLDDB: filter->set_threshold_db(value); break;
                case PORT_RATIO: filter->set_ratio(value); break;
            }
        }
    };
}