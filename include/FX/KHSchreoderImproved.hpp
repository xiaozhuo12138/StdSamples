...
class SchroederImproved
{
    ...
    static const int  NUM_LOWPASSES=1;
...
public:
    ...
    DspFloatType calcAPGain(const DspFloatType d_ms, const DspFloatType rt60);
...
//helper functions
inline DspFloatType SchroederImproved::calcCombGain(const DspFloatType d_ms, const DspFloatType rt60){
    return pow(10.0, ((-3.0 * d_ms) / (rt60 * 1000.0)));
    //(((return pow(10.0, (-3.0 * (d_ms*44100*.001) * (1.0/44100)) / (rt60 * 1000.0));

}

inline DspFloatType SchroederImproved::calcAPGain(const DspFloatType d_ms, const DspFloatType rt60){
    return pow(10.0, ((-3.0 * d_ms) / (rt60 * 1000.0)));
}
