...
namespace Analog::Filters::LadderFilter2
{
    // constants for ladder filter processing
    const DspFloatType TEMP = 30; 										///--> temperature in degrees Celsius
    const DspFloatType THERMAL_VOLT = 8.617f*1e-5*(273.15f + TEMP); 	///--> thermal voltage, Vt
    const DspFloatType OVER_TWO_THERMAL_VOLT = 1/(THERMAL_VOLT * 2.f); 	///--> precompute division to 2 * Vt
    const int NUMBER_OF_FILTERS = 4; 							///--> number of filter stages    
