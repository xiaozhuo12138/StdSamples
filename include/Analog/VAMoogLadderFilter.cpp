...
namespace Analog::Filters::MoogLadder
{
	...
	public:
		...
		FAUSTPP_VIRTUAL void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
			...
			DspFloatType fSlow1 = (0.0411641225f * (DspFloatType(fHslider1) + -0.707000017f));
			for (int i = 0; (i < count); i = (i + 1)) {
				fRec5[0] = (fSlow0 + (0.999000013f * fRec5[1]));
				DspFloatType fTemp0 = std::tan((fConst0 * fRec5[0]));
				DspFloatType fTemp1 = (3.9000001f - (0.899999976f * std::pow((0.0f - (0.333333343f * (1.0f - (std::log10(fRec5[0]) + -0.30103001f)))), 0.200000003f)));
				DspFloatType fTemp2 = (fTemp0 + 1.0f);
				DspFloatType fTemp3 = ((fTemp0 * (((DspFloatType(input0[i]) - (fSlow1 * (fTemp1 * (((fRec1[1] + (fRec2[1] * fTemp0)) + (mydsp_faustpower2_f(fTemp0) * fRec3[1])) + (mydsp_faustpower3_f(fTemp0) * fRec4[1]))))) / ((fSlow1 * (fTemp1 * mydsp_faustpower4_f(fTemp0))) + 1.0f)) - fRec4[1])) / fTemp2);
				DspFloatType fTemp4 = ((fTemp0 * ((fRec4[1] + fTemp3) - fRec3[1])) / fTemp2);
				DspFloatType fTemp5 = ((fTemp0 * ((fRec3[1] + fTemp4) - fRec2[1])) / fTemp2);
				DspFloatType fTemp6 = ((fTemp0 * ((fRec2[1] + fTemp5) - fRec1[1])) / fTemp2);
				DspFloatType fRec0 = (fRec1[1] + fTemp6);
	...
	void MoogLadder::init(DspFloatType sample_rate)
	{
	...
	const ParameterRange *MoogLadder::parameter_range(unsigned index) noexcept
	{
		switch (index) {
	...
	DspFloatType MoogLadder::get_parameter(unsigned index) const noexcept
	{
		mydsp &dsp = static_cast<mydsp &>(*fDsp);
		switch (index) {
	...
	void MoogLadder::set_parameter(unsigned index, DspFloatType value) noexcept
	{
		mydsp &dsp = static_cast<mydsp &>(*fDsp);
		switch (index) {
	...
	DspFloatType MoogLadder::get_cutoff() const noexcept
	{
		mydsp &dsp = static_cast<mydsp &>(*fDsp);
		return dsp.fHslider0;
	}

	DspFloatType MoogLadder::get_q() const noexcept
	{
		mydsp &dsp = static_cast<mydsp &>(*fDsp);
		return dsp.fHslider1;
	}
	...
	void MoogLadder::set_cutoff(DspFloatType value) noexcept
	{
		mydsp &dsp = static_cast<mydsp &>(*fDsp);
		dsp.fHslider0 = value;
	}

	void MoogLadder::set_q(DspFloatType value) noexcept
	{
		mydsp &dsp = static_cast<mydsp &>(*fDsp);
		dsp.fHslider1 = value;
	}
