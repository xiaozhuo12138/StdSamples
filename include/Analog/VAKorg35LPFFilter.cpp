 ...
 public:
	...
	FAUSTPP_VIRTUAL void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			DspFloatType fTemp1 = ((DspFloatType(input0[i]) - fRec3[1]) * fTemp0);
			DspFloatType fTemp2 = (fTemp0 + 1.0f);
			DspFloatType fTemp3 = (1.0f - (fTemp0 / fTemp2));
			DspFloatType fTemp4 = ((fTemp0 * ((((fRec3[1] + ((fTemp1 + (fSlow1 * (fRec1[1] * fTemp3))) / fTemp2)) + (fRec2[1] * (0.0f - (1.0f / fTemp2)))) / (1.0f - (fSlow1 * ((fTemp0 * fTemp3) / fTemp2)))) - fRec1[1])) / fTemp2);
			DspFloatType fTemp5 = (fRec1[1] + fTemp4);
...
void Korg35LPF::init(DspFloatType sample_rate)
{
...
const ParameterRange *Korg35LPF::parameter_range(unsigned index) noexcept
{
    switch (index) {
...
DspFloatType Korg35LPF::get_parameter(unsigned index) const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    switch (index) {
...
void Korg35LPF::set_parameter(unsigned index, DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    switch (index) {
...
DspFloatType Korg35LPF::get_cutoff() const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    return dsp.fHslider0;
}

DspFloatType Korg35LPF::get_q() const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    return dsp.fHslider1;
}
...
void Korg35LPF::set_cutoff(DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    dsp.fHslider0 = value;
}

void Korg35LPF::set_q(DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    dsp.fHslider1 = value;
}
