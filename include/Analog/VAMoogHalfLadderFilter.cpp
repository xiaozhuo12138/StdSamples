...
class mydsp : public dsp {
 ...
 public:
	...
	FAUSTPP_VIRTUAL void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		DspFloatType fSlow2 = (0.082328245f * fSlow1);
		for (int i = 0; (i < count); i = (i + 1)) {
			fRec4[0] = (fSlow0 + (0.999000013f * fRec4[1]));
			DspFloatType fTemp0 = std::tan((fConst0 * fRec4[0]));
			DspFloatType fTemp1 = (fTemp0 + 1.0f);
			DspFloatType fTemp2 = ((2.0f * (fTemp0 / fTemp1)) + -1.0f);
			DspFloatType fTemp3 = ((fTemp0 * (((DspFloatType(input0[i]) + (fSlow1 * (((0.0f - ((0.16465649f * fRec1[1]) + (0.082328245f * (fRec2[1] * fTemp2)))) - (0.082328245f * (((fTemp0 * fTemp2) * fRec3[1]) / fTemp1))) / fTemp1))) / ((fSlow2 * ((mydsp_faustpower2_f(fTemp0) * fTemp2) / mydsp_faustpower2_f(fTemp1))) + 1.0f)) - fRec3[1])) / fTemp1);
			DspFloatType fTemp4 = ((fTemp0 * ((fRec3[1] + fTemp3) - fRec2[1])) / fTemp1);
			DspFloatType fTemp5 = (fRec2[1] + fTemp4);
			DspFloatType fTemp6 = ((fTemp0 * (fTemp5 - fRec1[1])) / fTemp1);
			DspFloatType fRec0 = ((2.0f * (fRec1[1] + fTemp6)) - fTemp5);
...
void MoogHalfLadder::init(DspFloatType sample_rate)
{
...
const ParameterRange *MoogHalfLadder::parameter_range(unsigned index) noexcept
{
    switch (index) {
...
DspFloatType MoogHalfLadder::get_parameter(unsigned index) const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    switch (index) {
...
void MoogHalfLadder::set_parameter(unsigned index, DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    switch (index) {
...
DspFloatType MoogHalfLadder::get_cutoff() const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    return dsp.fHslider0;
}

DspFloatType MoogHalfLadder::get_q() const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    return dsp.fHslider1;
}
...
void MoogHalfLadder::set_cutoff(DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    dsp.fHslider0 = value;
}

void MoogHalfLadder::set_q(DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    dsp.fHslider1 = value;
}
