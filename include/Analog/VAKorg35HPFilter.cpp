...
class mydsp : public dsp {
 ...
 public:
	...
	FAUSTPP_VIRTUAL void compute(int count, FAUSTDspFloatType** inputs, FAUSTDspFloatType** outputs) {
		...
		DspFloatType fSlow1 = (0.215215757f * (DspFloatType(fHslider1) + -0.707000017f));
		for (int i = 0; (i < count); i = (i + 1)) {
			DspFloatType fTemp0 = DspFloatType(input0[i]);
			fRec4[0] = (fSlow0 + (0.999000013f * fRec4[1]));
			DspFloatType fTemp1 = std::tan((fConst0 * fRec4[0]));
			DspFloatType fTemp2 = ((fTemp0 - fRec3[1]) * fTemp1);
			DspFloatType fTemp3 = (fTemp1 + 1.0f);
			DspFloatType fTemp4 = (fTemp1 / fTemp3);
			DspFloatType fTemp5 = ((fTemp0 - (fRec3[1] + (((fTemp2 - fRec1[1]) - (fRec2[1] * (0.0f - fTemp4))) / fTemp3))) / (1.0f - (fSlow1 * ((fTemp1 * (1.0f - fTemp4)) / fTemp3))));
			DspFloatType fRec0 = fTemp5;
			DspFloatType fTemp6 = (fSlow1 * fTemp5);
			DspFloatType fTemp7 = ((fTemp1 * (fTemp6 - fRec2[1])) / fTemp3);
...
void Korg35HPF::init(DspFloatType sample_rate)
{
...
const ParameterRange *Korg35HPF::parameter_range(unsigned index) noexcept
{
    switch (index) {
...
DspFloatType Korg35HPF::get_parameter(unsigned index) const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    switch (index) {
...
void Korg35HPF::set_parameter(unsigned index, DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    switch (index) {
...
DspFloatType Korg35HPF::get_cutoff() const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    return dsp.fHslider0;
}

DspFloatType Korg35HPF::get_q() const noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    return dsp.fHslider1;
}
...
void Korg35HPF::set_cutoff(DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    dsp.fHslider0 = value;
}

void Korg35HPF::set_q(DspFloatType value) noexcept
{
    mydsp &dsp = static_cast<mydsp &>(*fDsp);
    dsp.fHslider1 = value;
}
