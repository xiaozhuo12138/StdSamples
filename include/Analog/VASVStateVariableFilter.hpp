...
namespace Analog::Filters::AnalogStateVariableFilter
{
    ...
    std::complex<DspFloatType> VirtualAnalogStateVariableFilter::calcTransferLowpass(DspFloatType w, DspFloatType wc, DspFloatType r)
    {
        ...
        return (wc * wc) / (s * s + (DspFloatType)2.0 * r * wc * s + wc * wc);
    }
    ...
    {
        std::complex<DspFloatType> s = w * std::complex<DspFloatType>(0, 1);
        return (wc * s) / (s * s + (DspFloatType)2.0 * r * wc * s + wc * wc);
    }
    ...
    {
        std::complex<DspFloatType> s = w * std::complex<DspFloatType>(0, 1);
        return (s * s) / (s * s + (DspFloatType)2.0 * r * wc * s + wc * wc);
    }
    ...
    {
        return (DspFloatType)2.0 * r * calcTransferBandpass(w, wc, r);
    }
    ...
    {
        return (DspFloatType)1.0 + k * calcTransferUnitGainBandpass(w, wc, r);
    }
    ...
    {
        return calcTransferBandShelving(w, wc, r, (DspFloatType)(-1.0));
    }
    ...
    struct VirtualAnalogStateVariableFilterProcessor : public FilterProcessorPlugin<VirtualAnalogStateVariableFilter>
    {
        ...
        void setCutoff(DspFloatType f) {
            this->setCutoffFreq(MusicFunctions::cv2freq(f));
        }
