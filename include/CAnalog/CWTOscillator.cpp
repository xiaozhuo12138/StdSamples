	...
	for(int j = 0; j < NUM_TABLES; j++)
	{
		...
		for(int i = 0; i < WT_LENGTH; i++)
		{
			...
			for(int g = 1; g <= nHarms; g++)
			{
				...
				pSawTable[i] += pow((DspFloatType)-1.0,(DspFloatType)(g+1))*(1.0/n)*sigma*sin(2.0*M_PI*i*n/WT_LENGTH);
			}
			...
			{
				DspFloatType n = DspFloatType(g);
				pTriTable[i] += pow((DspFloatType)-1.0, (DspFloatType)n)*(1.0/pow((DspFloatType)(2*n + 1),(DspFloatType)2.0))*sin(2.0*M_PI*(2.0*n + 1)*i/WT_LENGTH);
			}
