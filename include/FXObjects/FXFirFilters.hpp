	...
	*/
	virtual DspFloatType processAudioSample(DspFloatType xn)
	{
		...
		for (unsigned int i = 0; i < length; i++)
		{
			...
			DspFloatType signal = signalBuffer.readBuffer((int)i);
			DspFloatType irrrrr = irBuffer.readBuffer((int)i);
	...
	void setImpulseResponse(DspFloatType* irArray, unsigned int lengthPowerOfTwo)
	{
...
protected:
	...
	CircularBuffer<DspFloatType> signalBuffer; ///< circulat buffer for the signal
	LinearBuffer<DspFloatType> irBuffer;	///< linear buffer for the IR
...
public:
	...
	virtual bool reset(DspFloatType _sampleRate)
	{
		...
		memset(&analogMagArray[0], 0, sizeof(DspFloatType) * IR_LEN);	///< clear
		memset(&irArray[0], 0, sizeof(DspFloatType) * IR_LEN);	///< clear
...
private:
	...
	DspFloatType analogMagArray[IR_LEN]; ///< array for analog magnitude response
	DspFloatType irArray[IR_LEN]; ///< array to hold calcualted IR
	DspFloatType sampleRate = 0.0; ///< storage for sample rate
};
