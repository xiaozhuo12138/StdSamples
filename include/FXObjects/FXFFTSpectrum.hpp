...
@brief  creates a new std::unique_ptr<DspFloatType[]> array for a given window lenght and type.
...
*/
inline std::unique_ptr<DspFloatType[]> makeWindow(unsigned int windowLength, unsigned int hopSize, windowType window, DspFloatType& gainCorrectionValue)
{
	std::unique_ptr<DspFloatType[]> windowBuffer;
	windowBuffer.reset(new DspFloatType[windowLength]);

	if (!windowBuffer) return nullptr;

	DspFloatType overlap = hopSize > 0.0 ? 1.0 - (DspFloatType)hopSize / (DspFloatType)windowLength : 0.0;
...
public:
	...
	fftw_complex* doFFT(DspFloatType* inputReal, DspFloatType* inputImag = nullptr);

	/** do the IFFT and return real and imaginary arrays */
	fftw_complex* doInverseFFT(DspFloatType* inputReal, DspFloatType* inputImag);
...
protected:
	...
	DspFloatType* windowBuffer = nullptr;				///< buffer for window (naked)
	DspFloatType windowGainCorrection = 1.0;			///< window gain correction
...
*/
fftw_complex* FastFFT::doFFT(DspFloatType* inputReal, DspFloatType* inputImag)
{
...
\returns a pointer to a fftw_complex array: a 2D array of real (column 0) and imaginary (column 1) parts
*/
fftw_complex* FastFFT::doInverseFFT(DspFloatType* inputReal, DspFloatType* inputImag)
{
