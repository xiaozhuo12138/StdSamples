...
const DspFloatType TLD_AUDIO_ENVELOPE_ANALOG_TC = -0.99967234081320612357829304641019; // ln(36.7%)
...
struct AudioDetectorParameters
{
	...
	DspFloatType attackTime_mSec = 0.0; ///< attack time in milliseconds
	DspFloatType releaseTime_mSec = 0.0;///< release time in milliseconds
...
protected:
	...
	DspFloatType attackTime = 0.0;	///< attack time coefficient
	DspFloatType releaseTime = 0.0;	///< release time coefficient
	DspFloatType sampleRate = 44100;	///< stored sample rate
	DspFloatType lastEnvelope = 0.0;	///< output register

	/** set our internal atack time coefficients based on times and sample rate */
	void setAttackTime(DspFloatType attack_in_ms, bool forceCalc = false);

	/** set our internal release time coefficients based on times and sample rate */
	void setReleaseTime(DspFloatType release_in_ms, bool forceCalc = false);
};
...
struct DynamicsProcessorParameters
{
	...
	// --- individual parameters
	DspFloatType ratio = 50.0;				///< processor I/O gain ratio
	DspFloatType threshold_dB = -10.0;		///< threshold in dB
	DspFloatType kneeWidth_dB = 10.0;			///< knee width in dB for soft-knee operation
	...
	DspFloatType attackTime_mSec = 0.0;		///< attack mSec
	DspFloatType releaseTime_mSec = 0.0;		///< release mSec
	DspFloatType outputGain_dB = 0.0;			///< make up gain

	// --- outbound values, for owner to use gain-reduction metering
	DspFloatType gainReduction = 1.0;			///< output value for gain reduction that occurred
	DspFloatType gainReduction_dB = 0.0;		///< output value for gain reduction that occurred in dB
};
...
public:
	...
	virtual DspFloatType processAuxInputAudioSample(DspFloatType xn)
	{
	...
	*/
	virtual DspFloatType processAudioSample(DspFloatType xn)
	{
		...
		// --- compute gain
		DspFloatType gr = computeGain(detect_dB);

		// --- makeup gain
		DspFloatType makeupGain = pow(10.0, parameters.outputGain_dB / 20.0);
...
protected:
	...
	DspFloatType sidechainInputSample = 0.0; ///< storage for sidechain sample

	/** compute (and save) the current gain value based on detected input (dB) */
	inline DspFloatType computeGain(DspFloatType detect_dB)
	{
		DspFloatType output_dB = 0.0;
...
- Processes mono input to mono output.
Control I/F:
- setThreshold_dB(DspFloatType _threshold_dB) to adjust the limiter threshold
- setMakeUpGain_dB(DspFloatType _makeUpGain_dB) to adjust the makeup gain
\author Will Pirkle http://www.willpirkle.com
\remark This object is included in Designing Audio Effects Plugins in C++ 2nd Ed. by Will Pirkle
\version Revision : 1.0
\date Date : 2018 / 09 / 7
...
public:
	...
	*/
	...
	DspFloatType computeGain(DspFloatType detect_dB)
	{
		DspFloatType output_dB = 0.0;

		// --- defaults - you can change these here
		bool softknee = true;
		DspFloatType kneeWidth_dB = 10.0;
	...
	void setThreshold_dB(DspFloatType _threshold_dB) { threshold_dB = _threshold_dB; }

	/** adjust makeup gain in dB*/
	void setMakeUpGain_dB(DspFloatType _makeUpGain_dB) { makeUpGain_dB = _makeUpGain_dB; }

protected:
	AudioDetector detector;		///< the detector object
	DspFloatType threshold_dB = 0.0;	///< stored threshold (dB)
	DspFloatType makeUpGain_dB = 0.0;	///< stored makeup gain (dB)
};
...
*/
void AudioDetector::setAttackTime(DspFloatType attack_in_ms, bool forceCalc)
{
...
\param forceCalc flag to force a re-calculation of time constant even if values have not changed.
*/
void AudioDetector::setReleaseTime(DspFloatType release_in_ms, bool forceCalc)
{
