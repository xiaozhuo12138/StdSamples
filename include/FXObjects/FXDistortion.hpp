...
struct TriodeClassAParameters
{
	...
	DspFloatType saturation = 1.0;	///< saturation level
	DspFloatType asymmetry = 0.0;		///< asymmetry level
	DspFloatType outputGain = 1.0;	///< outputGain level
	...
	DspFloatType hpf_Fc = 1.0;		///< fc of DC blocking cap
	DspFloatType lsf_Fshelf = 80.0;	///< shelf fc from self bias cap
	DspFloatType lsf_BoostCut_dB = 0.0;///< boost/cut due to cathode self biasing
};
...
struct ClassATubePreParameters
{
	...
	// --- individual parameters
	DspFloatType inputLevel_dB = 0.0;		///< input level in dB
	DspFloatType saturation = 0.0;		///< input level in dB
	DspFloatType asymmetry = 0.0;			///< input level in dB
	DspFloatType outputLevel_dB = 0.0;	///< input level in dB

	// --- shelving filter params
	DspFloatType lowShelf_fc = 0.0;			///< LSF shelf frequency
	DspFloatType lowShelfBoostCut_dB = 0.0;	///< LSF shelf gain/cut
	DspFloatType highShelf_fc = 0.0;			///< HSF shelf frequency
	DspFloatType highShelfBoostCut_dB = 0.0;	///< HSF shelf frequency
...
public:
	...
	*/
	virtual DspFloatType processAudioSample(DspFloatType xn)
	{
		DspFloatType output1 = triodes[0].processAudioSample(xn*inputLevel);
		DspFloatType output2 = triodes[1].processAudioSample(output1);
		DspFloatType output3 = triodes[2].processAudioSample(output2);

		// --- filter stage is between 3 and 4
		DspFloatType outputEQ = shelvingFilter.processAudioSample(output3);
		DspFloatType output4 = triodes[3].processAudioSample(outputEQ);
...
protected:
	...
	DspFloatType inputLevel = 1.0;	///< input level (not in dB)
	DspFloatType outputLevel = 1.0;	///< output level (not in dB)
};
...
struct BitCrusherParameters
{
	...
	DspFloatType quantizedBitDepth = 4.0; ///< bid depth of quantizer
};
...
public:
	...
	/** reset members to initialized state */
	virtual bool reset(DspFloatType _sampleRate){ return true; }
...
protected:
	...
	DspFloatType QL = 1.0;				 ///< the quantization level
};
