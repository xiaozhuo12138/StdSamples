...
struct OscillatorParameters
{
	...
	DspFloatType frequency_Hz = 0.0;	///< oscillator frequency
};
...
protected:
	...
	// --- timebase variables
	DspFloatType modCounter = 0.0;			///< modulo counter [0.0, +1.0]
	DspFloatType phaseInc = 0.0;				///< phase inc = fo/fs
	DspFloatType modCounterQP = 0.25;			///<Quad Phase modulo counter [0.0, +1.0]

	/** check the modulo counter and wrap if needed */
	inline bool checkAndWrapModulo(DspFloatType& moduloCounter, DspFloatType phaseInc)
	{
	...
	inline bool advanceAndCheckWrapModulo(DspFloatType& moduloCounter, DspFloatType phaseInc)
	{
		// --- advance counter
	...
	inline void advanceModulo(DspFloatType& moduloCounter, DspFloatType phaseInc) { moduloCounter += phaseInc; }

	const DspFloatType B = 4.0 / kPi;
	const DspFloatType C = -4.0 / (kPi* kPi);
	...
	inline DspFloatType parabolicSine(DspFloatType angle)
	{
		DspFloatType y = B * angle + C * angle * fabs(angle);
		y = P * (y * fabs(y) - y) + y;
		return y;
	}
};
...
public:
	...
	{
		sampleRate = _sampleRate;
		memset(&stateArray[0], 0, sizeof(DspFloatType)*numDFOStates);
		updateDFO();
		return true;
	}
	...
	*/
	...
	void updateDFO()
	{
		...
		DspFloatType wT = (kTwoPi*parameters.frequency_Hz) / sampleRate;
		...
		DspFloatType wnT1 = asin(stateArray[df_yz1]);

		// find n by dividing wnT by wT
		DspFloatType n = wnT1 / wT;
...
protected:
	...
	DspFloatType stateArray[numDFOStates] = { 0.0, 0.0 };///< array of state registers
	DspFloatType coeffArray[numDFOCoeffs] = { 0.0, 0.0 };///< array of coefficients
...
const SignalGenData LFO::renderAudioOutput()
{
	...
	if (waveform == generatorWaveform::kSin)
	{
		...
		DspFloatType angle = modCounter*2.0*kPi - kPi;
