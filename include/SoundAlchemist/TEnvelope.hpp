...
namespace SoundWave::Envelopes
{
    ...
    public:
        ...
        TADSR2(T maxLevel = 10.0, unsigned int stepFrequency = 10000);
    ...
    template<typename T>
    TADSR2<T>::TADSR2(T maxLevel, unsigned int stepFrequency) {
    ...
    template<typename T>
    void TADSR2<T>::setAttackTime(T time) {
        parameters.attackTime = time;
    }

    template<typename T>
    void TADSR2<T>::setAttackSlope(T slope) {
        parameters.attackSlope = slope;
    }

    template<typename T>
    void TADSR2<T>::setDecayTime(T time) {
        parameters.decayTime = time;
    }

    template<typename T>
    void TADSR2<T>::setDecaySlope(T slope) {
        parameters.decaySlope = slope;
    }

    template<typename T>
    void TADSR2<T>::setReleaseTime(T time) {
        parameters.releaseTime = time;
    }

    template<typename T>
    void TADSR2<T>::setReleaseSlope(T slope) {
        parameters.releaseSlope = slope;
    }

    template<typename T>
    void TADSR2<T>::setSustainLevel(T level) {
        if (level>maxLevel) {
    ...
    template<typename T>
    typename TADSR2<T>::parameter_struct TADSR2<T>::getParameters() {
        return parameters;
    }

    template<typename T>
    typename TADSR2<T>::state_enum TADSR2<T>::getState() {
        return currentState;
    }
    ...
    template<typename T>
    T TADSR2<T>::getCurrentTime() {
        return currentStep / (T)stepFrequency;
    }

    template<typename T>
    void TADSR2<T>::onGateOn() {
        gotoState(attack);
    }

    template<typename T>
    void TADSR2<T>::onGateOff() {
        gotoState(release);
    }

    template<typename T>
    void TADSR2<T>::gotoState(state_enum newState) {
        previousState = currentState;
        currentState = newState;
    }

    template<typename T>
    T TADSR2<T>::calculateAttackValue(unsigned long currentStep, T time, T slope) {
        return std::pow(getCurrentTime() / time, std::pow(2.0, -slope));
    }

    template<typename T>
    T TADSR2<T>::calculateDecayValue(unsigned long currentStep, T time, T slope, T targetLevel) {
        return std::pow(getCurrentTime() / time, std::pow(2.0, -slope)) * (targetLevel - 1.0) + 1.0;
    }

    template<typename T>
    T TADSR2<T>::calculateReleaseValue(unsigned long currentStep, T time, T slope, T originLevel) {
