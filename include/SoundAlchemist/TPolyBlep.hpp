...
// it's the same exact thing as TPolyBLEP 
template<typename T>
class TPBLEP {
public:
    enum Waveform {
    ...
    TPBLEP(T sampleRate, Waveform waveform = SINE, T initialFrequency = 440.0);
...
template<typename T>
TPBLEP<T>::TPBLEP(T sampleRate, Waveform waveform, T initialFrequency)
...
template<typename T>
void TPBLEP<T>::setdt(T time) {
    freqInSecondsPerSample = time;
}

template<typename T>
void TPBLEP<T>::setFrequency(T freqInHz) {
    setdt(freqInHz / sampleRate);
}

template<typename T>
void TPBLEP<T>::setSampleRate(T sampleRate) {
...
template<typename T>
T TPBLEP<T>::getFreqInHz() const {
    return freqInSecondsPerSample * sampleRate;
}

template<typename T>
void TPBLEP<T>::setPulseWidth(T pulseWidth) {
    this->pulseWidth = pulseWidth;
}

template<typename T>
void TPBLEP<T>::sync(T phase) {
    t = phase;
    if (t >= 0) {
        t -= bitwiseOrZero(t);
    } else {
...
template<typename T>
void TPBLEP<T>::setWaveform(Waveform waveform) {
    this->waveform = waveform;
}

template<typename T>
T TPBLEP<T>::get() const {
...
using PBLEP32 = SoundAlchemy::Oscillators::TPBLEP<float>;
using PBLEP64 = SoundAlchemy::Oscillators::TPBLEP<double>;