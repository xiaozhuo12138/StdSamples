...
namespace SoundAlchemy::Noise
{
    ...
    struct TGaussianWhiteNoise {
        ...
        void process(size_t n, std::vector<T> & s) {
            s.resize(n);
        ...
        void Process(T * samples, size_t n) {
            for(size_t i = 0; i < n; i++) samples[i] = Generate();        
        }
    };

    template<typename T>
    struct TModifiedVelvetNoise {
        ...
        TModifiedVelvetNoise(int fs = 48000) {
            base_fs = fs;
            length_list[0] = 152;
            length_list[1] = 400;
    ...
    public:
        ...
        TPinkNoiseGenerator(int numRows = 12) {
            pinkIndex = 0;
    ...
    private:
        ...
        typename std::vector<T>::iterator itB;
    ...
    public:
        // constructor
        TBrownNoiseGenerator(int bL = 20000) {
            bLength = bL;
