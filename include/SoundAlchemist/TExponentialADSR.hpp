...
struct ExponentialADSR {
    ...
    T tan2(T x,T g=1)       { return clamp(2*(2/M_PI*std::tan((2/M_PI)*g*x)),-1,1); }
...
inline T ExponentialADSR<T>::process() {
    ...
    else if(type == SOFTLOG) output *=  -(std::exp(output)-1)/(1+std::exp(output))*(std::log(output/3.6));
...
template<typename T>
ExponentialADSR<T>::ExponentialADSR(T fs, T ar, T dr, T sl, T rr, T tra, T trdr) {
    reset();
    sr = fs;
    setAttackRate(ar);
    setDecayRate(dr);
...
template<typename T>
ExponentialADSR<T>::~ExponentialADSR(void) {
}
