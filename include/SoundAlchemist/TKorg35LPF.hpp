...
{    
    ...
    public:
        ...
        FAUSTPP_VIRTUAL void compute(int count, T** inputs, T** outputs) {
            ...
            for (int i = 0; (i < count); i = (i + 1)) {
                ...
                T fTemp1 = ((T(input0[i]) - fRec3[1]) * fTemp0);
                T fTemp2 = (fTemp0 + 1.0f);
                T fTemp3 = (1.0f - (fTemp0 / fTemp2));
                T fTemp4 = ((fTemp0 * ((((fRec3[1] + ((fTemp1 + (fSlow1 * (fRec1[1] * fTemp3))) / fTemp2)) + (fRec2[1] * (0.0f - (1.0f / fTemp2)))) / (1.0f - (fSlow1 * ((fTemp0 * fTemp3) / fTemp2)))) - fRec1[1])) / fTemp2);
                T fTemp5 = (fRec1[1] + fTemp4);
                T fRec0 = fTemp5;
        ...
        FAUSTPP_VIRTUAL void tick(T& inputs, T& outputs) {
            ...
            fRec4[0] = (fSlow0 + (0.999000013f * fRec4[1]));
            T fTemp0 = std::tan((fConst0 * fRec4[0]));
            T fTemp1 = ((inputs - fRec3[1]) * fTemp0);
            T fTemp2 = (fTemp0 + 1.0f);
            T fTemp3 = (1.0f - (fTemp0 / fTemp2));
            T fTemp4 = ((fTemp0 * ((((fRec3[1] + ((fTemp1 + (fSlow1 * (fRec1[1] * fTemp3))) / fTemp2)) + (fRec2[1] * (0.0f - (1.0f / fTemp2)))) / (1.0f - (fSlow1 * ((fTemp0 * fTemp3) / fTemp2)))) - fRec1[1])) / fTemp2);
            T fTemp5 = (fRec1[1] + fTemp4);
    ...
    template<typename T>
    void TKorg35LPF<T>::init(T sample_rate)
    {
    ...
    template<typename T>
    void TKorg35LPF<T>::clear() noexcept
    {
    ...
    template<typename T>
    const char *TKorg35LPF<T>::parameter_label(unsigned index) noexcept
    {
        switch (index) {
    ...
    template<typename T>
    const char *TKorg35LPF<T>::parameter_short_label(unsigned index) noexcept
    {
        switch (index) {
    ...
    template<typename T>
    const char *TKorg35LPF<T>::parameter_symbol(unsigned index) noexcept
    {
        switch (index) {
    ...
    template<typename T>
    const char *TKorg35LPF<T>::parameter_unit(unsigned index) noexcept
    {
        switch (index) {
    ...
    template<typename T>
    const typename TKorg35LPF<T>::ParameterRange *TKorg35LPF<T>::parameter_range(unsigned index) noexcept
    {
        switch (index) {
        
        case 0: {
            static const typename TKorg35LPF<T>::ParameterRange range = { 20000, 20, 20000 };
            return &range;
        }
        
        case 1: {
            static const typename TKorg35LPF<T>::ParameterRange range = { 1, 0.5, 10 };
            return &range;
        }
    ...
    template<typename T>
    bool TKorg35LPF<T>::parameter_is_trigger(unsigned index) noexcept
    {
        switch (index) {
    ...
    template<typename T>
    bool TKorg35LPF<T>::parameter_is_boolean(unsigned index) noexcept
    {
        switch (index) {
    ...
    template<typename T>
    bool TKorg35LPF<T>::parameter_is_integer(unsigned index) noexcept
    {
        switch (index) {
    ...
    template<typename T>
    bool TKorg35LPF<T>::parameter_is_logarithmic(unsigned index) noexcept
    {
        switch (index) {
    ...
    template<typename T>
    T TKorg35LPF<T>::get_parameter(unsigned index) const noexcept
    {
        mydsp<T> &dsp = static_cast<mydsp<T> &>(*fDsp);
        switch (index) {
    ...
    template<typename T>
    void TKorg35LPF<T>::set_parameter(unsigned index, T value) noexcept
    {
        mydsp<T> &dsp = static_cast<mydsp<T> &>(*fDsp);
        switch (index) {
    ...
    template<typename T>
    T TKorg35LPF<T>::get_cutoff() const noexcept
    {
        mydsp<T> &dsp = static_cast<mydsp<T> &>(*fDsp);
        return dsp.fHslider0;
    }
    ...
    template<typename T>
    T TKorg35LPF<T>::get_q() const noexcept
    {
        mydsp<T> &dsp = static_cast<mydsp<T> &>(*fDsp);
        return dsp.fHslider1;
    }
    ...
    template<typename T>
    void TKorg35LPF<T>::set_cutoff(T value) noexcept
    {
        mydsp<T> &dsp = static_cast<mydsp<T> &>(*fDsp);
        dsp.fHslider0 = value;
    }
    ...
    template<typename T>
    void TKorg35LPF<T>::set_q(T value) noexcept
    {
        mydsp<T> &dsp = static_cast<mydsp<T> &>(*fDsp);
        dsp.fHslider1 = value;
    }
