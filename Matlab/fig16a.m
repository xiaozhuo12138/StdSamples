% fig16a.m - step gain, linear bandwidth
%
% -------------------------------------------------------------------------
...
for n=1:N3+1,
  if n<=N1+1,                                    
     ...
     G(n) = G1; 
  elseif n<=N2+1,
     Df(n) = Df1 + (Df2-Df1)/(N2-N1) * (n-N1-1);   % linear change
     G(n) = G2;                                    % step change
  else
     Df(n) = Df2;                          
     G(n) = G2;
   end
  if G(n)<gB,                                      % determine GB
     GB(n) = G(n)/2;                               % GB = sqrt(G(n)) in absolute units
  else
     GB(n) = G(n)-gB;                              % gB dB below G
     if type==2, GB(n) = gB; end                   % gB dB above G0
  end
  if GB(n)<gs,                                     % determine Gs
     Gs(n) = GB(n)/2;              
  else
     Gs(n) = gs; 
  end
