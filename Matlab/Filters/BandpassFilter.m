function FIR_bandpass = BandpassFilter(As, Fs, fs1, fp1, fp2, fs2, Kaiser)
%FIR Peak Filter design
% ARGUMENTS
%           As: stopband attenuation - dB (scalar)
%           Fs: sampling frequency - Hz (scalar)
%           fs1: first stopband edge - Hz (scalar)
%           fp1: first passband edge - Hz (scalar)
%           fp2: second passband edge - Hz (scalar)
%           fs2: second stopband edge - Hz (scalar)
%           Kaiser: flag for using Kaiser window - 1 or 0 (scalar)
% RETURNS
%           FIR_bandpass: FIR bandpass filter coefficients
...
Tb = 2*pi*(min((fs2 - fp2), (fp1 - fs1)))/Fs;
...
for n = 0 : N-1
    if n == alpha
        h_id(n+1) = 2*(fc2/Fs - fc1/Fs);
    else
        h_id(n+1) = (sin(2*pi*fc2/Fs*(n-alpha)) - sin(2*pi*fc1/Fs*(n-alpha)))./(pi*(n-alpha));
    end
end
