% hpeqord_a.m - order determination of analog parametric equalizer
%
% Usage: [N,k,k1] = hpeqord_a(G0,G,GB,Gs,Df,Dfs,type,Gb); 
%
%        [N,k,k1] = hpeqord_a(G0,G,GB,Gs,Df,Dfs);          Butterworth
%        [N,k,k1] = hpeqord_a(G0,G,GB,Gs,Df,Dfs,0);        Butterworth
%        [N,k,k1] = hpeqord_a(G0,G,GB,Gs,Df,Dfs,1);        Chebyshev-1
%        [N,k,k1] = hpeqord_a(G0,G,GB,Gs,Df,Dfs,2);        Chebyshev-2
%        [N,k,k1] = hpeqord_a(G0,G,GB,Gs,Df,Dfs,3);        Elliptic
%        [N,k,k1] = hpeqord_a(G0,G,GB,Gs,Df,Dfb,3,Gb);     Elliptic, Dwb at Gb
%
% G0  = reference gain (all gains must be in dB, enter -Inf to get 0 in absolute units)
% G   = peak/cut gain 
% GB  = passband bandwidth gain
% Gs  = stopband gain
% Df  = bandwidth in Hz at level GB
% Dfs = bandwidth in Hz at level Gs
...
%              G>GB>Gs>G0 and Df<Dfs, for a boost   (elliptic Gb case: G>GB>Gb>Gs>G0)
%              G<GB<Gs<G0 and Df<Dfs, for a cut
%        for Chebyshev-2:
%              G>Gs>GB>G0 and Df>Dfs, for a boost
%              G<Gs<GB<G0 and Df>Dfs, for a cut
%
%        given the bandedge frequencies [f1,f2] and [f1s,f2s], use the following steps to design the filter:
%
%           Df = f2-f1; Dfs = f2s-f1s;
%           N = hpeqord_a(G0,G,GB,Gs,Df,Dfs,type,Gb); N = ceil(N);
%           f0 = sqrt(f1*f2), or, f0 = sqrt(f1s*f2s)
%           [B,A,Ba,Aa] = hpeqa(N,G0,G,GB,f0,Df,type,Gs,tol);
%
%         for highpass designs, must invert Df,Dfs, e.g., N=hpeqord_a(G0,G,GB,Gs,1/Df,1/Dfs,3,Gb);
% 
...
function [N,k,k1] = hpeqord_a(G0, G, GB, Gs, Df, Dfs, type, Gb)

if nargin==0, help hpeqord_a; return; end
