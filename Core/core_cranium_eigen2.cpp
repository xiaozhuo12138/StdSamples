...
{
    return Matrix<T>(a.cwiseProduct(b));
}
...
struct Layer {
    ...
    void train_output(Matrix<float>& target, Connection * cconnect, Connection* pconnect) {        
        errori = input - target;
        pconnect->dWi = pconnect->from->input.transpose()  * errori;
        pconnect->dbi = errori;        
    }
    void train_hidden(Layer * next, Layer * prev, Connection * cconnect, Connection * nconnect, Connection * pconnect) {                   
        prev->errorLastTi = next->errori * cconnect->weights.transpose();
...
template<typename T>
Matrix<T> addToEachRow(const Matrix<T>& m, const Matrix<T> & v)
{
    Matrix<T> r(m);
    for(size_t i = 0; i < m.rows(); i++)
        for(size_t j = 0; j < m.cols();j++)
