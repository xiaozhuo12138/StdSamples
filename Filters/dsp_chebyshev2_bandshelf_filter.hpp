...
namespace Filters
{
    struct ChebyshevIIBandShelfFilter : public FilterBase
    {
        Dsp::ChebyshevII::BandShelf<32> prototype;

        ChebyshevIIBandShelfFilter(size_t Order, DspFloatType BW, DspFloatType G, DspFloatType stopdb, DspFloatType Fc, DspFloatType Fs)
        {            
            order = Order;
            sr    = Fs;
            stop=stopdb;
