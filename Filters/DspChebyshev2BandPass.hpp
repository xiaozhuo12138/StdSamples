...
namespace Filters
{
    struct ChebyshevIIBandPassFilter : public FilterBase
    {
        Dsp::ChebyshevII::BandPass<32> prototype;
                
        ChebyshevIIBandPassFilter(size_t Order, DspFloatType BW, DspFloatType stopBand, DspFloatType Fc, DspFloatType Fs)
        {            
            order = Order;
            sr    = Fs;
            stop  = stopBand;
