...
namespace Filters
{
    struct ChebyshevIIHighShelfFilter : public FilterBase
    {
        Dsp::ChebyshevII::HighShelf<32> prototype;
                
        ChebyshevIIHighShelfFilter(size_t Order, DspFloatType G, DspFloatType stopdb, DspFloatType Fc, DspFloatType Fs)
        {            
            order = Order;
            sr    = Fs;
            stop = stopdb;
