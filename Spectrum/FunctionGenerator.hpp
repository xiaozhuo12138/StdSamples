...
namespace Oscillators::Generators
{
    ...
    struct SineFunction : public GeneratorProcessor
    {
        ...
        SineFunction(double f, double sr = 44100): GeneratorProcessor(), frequency(f), startphase(0), phase(0.0), sampleRate(sr) {}
        ...
        // Y = phase modulation
        double Tick(double Index=1, double A = 1, double X = -1, double Y = 1) {
            double r = A*function();
            double p = phase;
    ...
    struct PhasorFunction : public GeneratorProcessor
    {
        enum Polarity 
        {
            BIPOLAR,
        ...
        PhasorFunction(double f, double sr = 44100) : GeneratorProcessor(), frequency(f),phase(0.0), sampleRate(sr) {}
        ...
        double Tick(double I=1, double A = 1, double X = -1, double Y = 1) {
            double r = A*phase;
            double p = phase;
            phase = phasor(I*(frequency + X*frequency)) + (Y*phase);
            phase = std::fmod(phase,1);        
            if(polarity == POSITIVE) r = std::abs(r);
            else if(polarity == NEGATIVE) r = -std::abs(r);
            phase = p;
...
namespace Oscillators::Functions
{
    struct FunctionGenerator : public GeneratorProcessor
    {
        ...
        FunctionGenerator(double f, double sr=44100) : GeneratorProcessor(), frequency(f),sampleRate(sr),startphase(0),phase(0.0) {
            inc = f/sr;
        }
