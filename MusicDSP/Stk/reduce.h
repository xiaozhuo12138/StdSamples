...
template<typename T>
__global__
void block_sum_reduce(T* const d_block_sums, 
...
template<typename T>
__global__ void reduce0(float* g_odata, unsigned int* g_idata, unsigned int len) {
...
template<typename T>
__global__ void reduce1(float * g_odata, unsigned int* g_idata, unsigned int len) {
...
template<typename T>
__global__ void reduce2(T* g_odata, unsigned int* g_idata, unsigned int len) {
	extern __shared__ T sdata[];
...
template<typename T>
void print_d_array(T* d_array, unsigned int len)
{
	unsigned int* h_array = new T[len];
	checkCudaErrors(cudaMemcpy(h_array, d_array, sizeof(T) * len, cudaMemcpyDeviceToHost));
...
template<typename T>
T gpu_sum_reduce(T* d_in, unsigned int d_in_len)
{
	...
	checkCudaErrors(cudaMalloc(&d_block_sums, sizeof(T) * grid_sz));
	checkCudaErrors(cudaMemset(d_block_sums, 0, sizeof(T) * grid_sz));

	// Sum data allocated for each block
	block_sum_reduce<<<grid_sz, block_sz, sizeof(T) * max_elems_per_block>>>(d_block_sums, d_in, d_in_len);
	...
	{
		T* d_total_sum;
		checkCudaErrors(cudaMalloc(&d_total_sum, sizeof(T)));
		checkCudaErrors(cudaMemset(d_total_sum, 0, sizeof(T)));
		block_sum_reduce<<<1, block_sz, sizeof(T) * max_elems_per_block>>>(d_total_sum, d_block_sums, grid_sz);
		//reduce4<<<1, block_sz, sizeof(unsigned int) * block_sz>>>(d_total_sum, d_block_sums, grid_sz);
		checkCudaErrors(cudaMemcpy(&total_sum, d_total_sum, sizeof(T), cudaMemcpyDeviceToHost));
		checkCudaErrors(cudaFree(d_total_sum));
	}
	else
	{
		T* d_in_block_sums;
		checkCudaErrors(cudaMalloc(&d_in_block_sums, sizeof(T) * grid_sz));
		checkCudaErrors(cudaMemcpy(d_in_block_sums, d_block_sums, sizeof(T) * grid_sz, cudaMemcpyDeviceToDevice));
