...
function array_new(size)
    local a = {} 
    a.array = luakissfft.float_vector(size)
    ...
    a.copy = function(a,b) 
        a.array = luakissfft.float_vector(b:size())        
        for i=1,b:size() do 
...
function sq_ConfigureFFT(q)
    ...
    q.fft_in  = luakissfft.complex_vector(q.frame_size)
    q.fft_out = luakissfft.complex_vector(q.frame_size)    
    q.fft = luakissfft.KissFFT(q.frame_size)
    q.fft_configured = true     
end 

function sq_FFT(q)        
    for i=0,q.frame_size-1 do                 
        local c = luakissfft.kiss_fft_cpx()
