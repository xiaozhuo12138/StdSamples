...
fvar fullwave_sigmoid_d(fvar f)
{
    return 2.0*(1.0 / (1.0 + exp(-f)))-1.0;    
}
...
fvar tanh_func_d(fvar f) { return tanh(f); }
...
fvar atan_func_d(fvar f) { return atan(f); }
...
fvar algebra_d(fvar f)
{
    return f / (sqrt(1.0 + f*f));
}
...
void fullwave_relu(Matrix & m)
{    
    m = m.cwiseMax(0).eval();            
    m = 2 * m.array()-1;
}
fvar fullwave_relu_d(fvar f)
{    
    return 2*f-1;
}
    ...
    Layer(LayerType t, size_t s, ActivationType a) {
        ...
        switch(a) {
            ...
            case FULLWAVE_SIGMOID: activate_f = fullwave_sigmoid;
                          ...
                          deriv = fullwave_sigmoid_d;
                          break;                          
            case FULLWAVE_RELU:  
                        activate_f = fullwave_relu;
                        activate_grad_f = tanh_grad;
                        deriv      = fullwave_relu_d;
                        break;
...
public:    
    ...
    floatType MeanSquaredError(Matrix& prediction, Matrix & actual, floatType rs) {
        floatType total_err = 0;
        floatType reg_err = 0;
