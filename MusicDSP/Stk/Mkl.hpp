...
namespace MKL
{
	...
	*/
        ...
        void random(T min = T(0), T max = T(1)) {
            /*
            Std::RandomMersenne r;
            for(size_t i = 0; i < size(); i++) vector[i] = r.uniform_real_distribution(min,max);
            */
            for(size_t i = 0; i < size(); i++) vector[i] = (T)rand() / (T)RAND_MAX;
        }
    ...
    struct Matrix
    {
        ...
        void random(T min = T(0), T max = T(1)) {
            //Std::RandomMersenne r;
            //for(size_t i = 0; i < size(); i++) matrix[i] = r.uniform_real_distribution(min,max);
            for(size_t i = 0; i < size(); i++) matrix[i] = (T)rand() / (T)RAND_MAX;
        }
        void identity() {
            size_t x = 0;
            zero();
