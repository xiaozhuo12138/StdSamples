...
%inline %{
    ...
    float randint(int a, int b) { return (b-a)*randf()+a; }
%}
%inline %{
    ...
    {        
        ...
        void load(const char * filename) {
            ...
            assert(f != NULL);
            //size_t n = fread(&size,sizeof(int),1,f);            
            size  = fmsynth_preset_size();
            buffer = (uint8_t*)malloc(size);            
            int n = fread(buffer,sizeof(uint8_t),size,f);            
            fclose(f);
        }        
    };
    ...
    struct FMSynth
    {
        ...
        void randomize() {            
            ...
            {
                set_parameter(FMSYNTH_PARAM_AMP,i,randf());
                set_parameter(FMSYNTH_PARAM_PAN,i,randf());
                set_parameter(FMSYNTH_PARAM_FREQ_MOD,i,randf());
                set_parameter(FMSYNTH_PARAM_FREQ_OFFSET,i,randf());
                set_parameter(FMSYNTH_PARAM_ENVELOPE_TARGET0,i,randf());
                set_parameter(FMSYNTH_PARAM_ENVELOPE_TARGET1,i,randf());
                set_parameter(FMSYNTH_PARAM_ENVELOPE_TARGET2,i,randf());
                set_parameter(FMSYNTH_PARAM_DELAY0,i,randf());
                set_parameter(FMSYNTH_PARAM_DELAY1,i,randf());
                set_parameter(FMSYNTH_PARAM_DELAY2,i,randf());
                set_parameter(FMSYNTH_PARAM_RELEASE_TIME,i,randf());
                set_parameter(FMSYNTH_PARAM_VELOCITY_SENSITIVITY,i,randf());
                set_parameter(FMSYNTH_PARAM_MOD_WHEEL_SENSITIVITY,i,randf());
                set_parameter(FMSYNTH_PARAM_LFO_AMP_SENSITIVITY,i,randf());
                set_parameter(FMSYNTH_PARAM_LFO_FREQ_MOD_DEPTH,i,randf());
            }
        }
