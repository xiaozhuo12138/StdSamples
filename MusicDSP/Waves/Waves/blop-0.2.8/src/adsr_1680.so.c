...
void connectPortAdsr(LADSPA_Handle instance,
                     unsigned long port,
                     LADSPA_Data *data)
{
    ...
    if (adsr_descriptors) {
        for (i = 0; i < ADSR_VARIANT_COUNT; i++) {
            ...
            if (descriptor) {
                ...
                descriptor->Label = "adsr_g+t";
                descriptor->Properties = LADSPA_PROPERTY_HARD_RT_CAPABLE;
                descriptor->Name = G_("ADSR Envelope with Gate and Trigger");
                ...
                /* Parameters for Gate */
                port_descriptors[ADSR_GATE] = gate_port_descriptors[i];
                port_names[ADSR_GATE] = G_("Gate");
                port_range_hints[ADSR_GATE].HintDescriptor = LADSPA_HINT_TOGGLED;

                /* Parameters for Trigger */
                port_descriptors[ADSR_TRIGGER] = trigger_port_descriptors[i];
                port_names[ADSR_TRIGGER] = G_("Trigger");
                port_range_hints[ADSR_TRIGGER].HintDescriptor = LADSPA_HINT_TOGGLED;
