...
public:
    ...
    */
    ...
    // debug
    static void print(std::string name, matrix<double>& M);
    static void print(std::string name, vector<double>& V);

private:
    double initialValue = 0.0;
    double prevInput = 0.0;

    // Ìàòðèöû ïðîñòðàíñòâà ñîñòîÿíèé
    matrix<double> A;
    matrix<double> B;
    matrix<double> C;
    matrix<double> D;

    // Ìàòðèö äëÿ ðàññ÷åòà âûõîäíîãî ñèãíàëà
    matrix<double> xout;
    matrix<double> M;
    matrix<double> expM;
    matrix<double> Ad;
...
void TransferFunction::print(std::string name, matrix<double>& M)
{
    int width = 10;
    int size1 = M.size1();
    int size2 = M.size2();
...
void TransferFunction::print(std::string name, vector<double>& V)
{
